...
*   **代表成果**：
    ...
    *   **专家系统 (Expert Systems, 1980s)**：如医疗咨询系统 MYCIN；Deep Blue 则结合搜索、评估函数、专用硬件与人工编码棋类知识，不属于典型专家系统。
*   **数学基础**：集合论、图论、数理逻辑。
*   **局限性**：
    ...
    *   <span style="background-color: #F8CECC; color: black; padding: 2px 4px; border-radius: 4px;">脆性 (Brittleness)</span>：知识库覆盖不足或前提变化时，规则系统可能无法平滑泛化；具体程度取决于不确定性建模和后备机制。
    *   <span style="background-color: #F8CECC; color: black; padding: 2px 4px; border-radius: 4px;">符号落地问题 (Symbol Grounding Problem)</span>：需要解释内部符号如何通过感知、行动或社会使用与外部对象和意义建立联系，而不是只在符号之间循环定义。
...
*   **核心主张**：大量能力可通过 **调整连接权重 (Weight Adjustment)** 从数据中 **学习 (Learning)**；网络结构、目标函数和数据流程仍包含人工设定的归纳偏置。
*   **处理流程示意**：
...
*   **发展历程**：
    ...
    *   **低潮 (约 1969-1986)**：Minsky 与 Papert 系统分析了单层感知机的表示局限；资金、算力、数据与整个 AI 领域的预期落差也共同影响了连接主义研究。
    *   **复兴 (1986-1995)**：Hinton 等人推广反向传播 (Backpropagation)，多层感知机 (MLP) 兴起。
    *   **规模化发展 (2012-2026)**：深度学习 (Deep Learning) 结合大数据与算力，在视觉、语音、自然语言和生成建模等领域成为主流方法。
