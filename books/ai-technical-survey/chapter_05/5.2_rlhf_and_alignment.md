...
<img src="images/post_training_pipeline.svg" width="95%" />
...
RLHF 及其后续偏好优化方法显著提升了模型的可用性和指令遵循能力，也是安全对齐工程中的重要组成部分。但它不是完整的后训练体系；事实校验、拒答策略、工具权限、红队测试和上线监控仍然不可替代。关于 2024 年之后更完整的后训练管线、推理强化、可验证奖励和数据闭环，见 **[5.5 现代后训练](5.5_modern_post_training.md)**。
