...
这也是开放模型社区能够快速繁殖出大量模型变体的原因之一：一个公开底座可以挂载多个 LoRA，用于代码、角色、翻译、医学、数学或工具调用。LoRA 既可以作为独立 adapter 分发，也可以在发布前 merge 回底模权重。关于它在开放权重生态中的位置，见 **[5.8 开放权重模型生态](5.8_open_weight_model_ecosystem.md)**。
