...
*   **视频扩散 Transformer**: Sora、Veo 等系统把图像生成推进到更长时序的视频生成。它们展示了更强的角色一致性和镜头连续性，但仍可能在因果、空间关系和物理交互上失败。关于图像/视频生成和世界模型的系统讨论，见 **[6.6 世界模型与生成式媒体](6.6_world_models_and_generative_media.md)**。
...
下一步的关键不是单纯“看得更多”，而是能否在多模态输入中保持一致的世界模型、稳定的时间推理和可靠的行动反馈。相关延伸见 **[6.6 世界模型与生成式媒体](6.6_world_models_and_generative_media.md)**。
