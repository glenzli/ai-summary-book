...
一个二元乘法只是单个函数，operad 却要同时记录零元常数、一元变换、任意多元运算、输入重标号以及所有嵌套代入。若不先固定集合论大小、零元 arity 和对称作用的口径，同一句“这个 operad 控制交换代数”可能在不同文献中指向不同对象。本序章据此确定全书默认语境，并说明定义、证明、外部输入与研究问题各自承担什么责任。读者只需熟悉集合、函数、有限群、商集、范畴、函子、自然变换和幺半范畴；模型范畴、同伦代数与 infinity-范畴将在真正需要它们时再建立。
...
第二十一章按这些数学障碍组织开放问题，而不按论文年份编排。凡作为外部输入调用的结果都必须固定模型、假设与来源；只用于说明研究方向的材料不进入前二十章的证明链。

## 0.5 从运算问题进入代入语言

上述约定把后文的起点压缩为一个明确问题：怎样在允许空输入的同时，把有限输入集的重标号和嵌套代入写成真正的幺半结构。第一章将直接构造这个代入乘积；在模型结构尚未出现之前，所有等式都仍是集合或范畴中的严格等式。到同伦与 infinity 语境中，相同问题会改由弱等价、提升性质和相干数据回答，而不是把基础定义含混地“升级”。
