...
本章引入非对称 operad（nonsymmetric operad）和偏复合（partial composition）。它们把第一章的整体代入拆成一次只在一个输入槽中代入的基本操作。随后用平面有根树说明：operad 公理正是树收缩次序无关性的代数化表达。

## 依赖前置知识

需要第一章的 arity 写法、operad 单位和代入结合律。树论只需有限图的基本语言。
...
非对称 operad 是没有对称群重标号的 operad。偏复合 $\circ_i$ 是其最小操作单位；整体代入可以由偏复合迭代恢复。平面有根树给出偏复合公理的几何记账方式：operad 结合律正是装饰树的收缩顺序无关性。
