...
- Jean-Louis Loday and Bruno Vallette, *Algebraic Operads*, Grundlehren der mathematischen Wissenschaften 346, Springer, 2012. 用途：线性 operad、Koszul 对偶、bar-cobar 构造和同伦代数的主参考。
...
- Dmitri Pavlov and Jakob Scholbach, “Admissibility and rectification of colored symmetric operads,” arXiv:1410.5675, 2014. 用途：colored symmetric operads 的 admissibility 与 rectification 条件。链接：<https://arxiv.org/abs/1410.5675>。
...
- Gijs Heuts, Vladimir Hinich, and Ieke Moerdijk, “On the equivalence between Lurie's model and the dendroidal model for infinity-operads,” arXiv:1305.3658; *Advances in Mathematics*. 用途：dendroidal 与 Lurie infinity-operad 模型比较。链接：<https://arxiv.org/abs/1305.3658>。
...
- Jacob Lurie, *Higher Algebra*. 用途：Lurie-style infinity-operad、operadic fibration、higher algebra。作者 PDF：<https://www.math.ias.edu/~lurie/papers/HA.pdf>。
...
- David Ayala and John Francis, “Factorization homology of topological manifolds,” *Journal of Topology*, 2015; arXiv:1206.5522v6. 用途：factorization homology、excision、圆周 Hochschild 计算和 manifold gluing。链接：<https://arxiv.org/abs/1206.5522>。
...
- [LABEL_LEDGER_CH08_21.md](LABEL_LEDGER_CH08_21.md) 不新增数学定理；它把第八至第二十一章的 413 个正式编号项登记为稳定交叉引用 label。
- [LABEL_LEDGER_REMAINING_APPENDICES.md](LABEL_LEDGER_REMAINING_APPENDICES.md) 不新增数学定理；它把剩余附录 C/D/E/F/G/I/J/L/M/N/O/Q/R/S/T/V/W/Y/Z 的 395 个正式编号项登记为稳定交叉引用 label。
...
- [P0_REFERENCE_LOCATORS_BATCH_3.md](P0_REFERENCE_LOCATORS_BATCH_3.md) 不新增数学定理；它记录 Ayala-Francis factorization homology、excision、圆周 Hochschild 计算和带边界版本的 P0 精确定位。
- [P0_REFERENCE_LOCATORS_BATCH_4.md](P0_REFERENCE_LOCATORS_BATCH_4.md) 不新增数学定理；它记录 Ginzburg-Kapranov classical quadratic Koszul duality 的 P0 精确定位；modern bar-cobar 版本由批次 5 和 final closure 分类处理。
- [P0_REFERENCE_LOCATORS_BATCH_5.md](P0_REFERENCE_LOCATORS_BATCH_5.md) 不新增数学定理；它记录 Fresse operadic cobar/cofibrant replacement 和 Hinich dg-operad model context 的 P0 精确定位，并把 Loday-Vallette/Fresse 书本紧凑表述归入 final convention translation。
...
- [P0_REFERENCE_LOCATORS_BATCH_9.md](P0_REFERENCE_LOCATORS_BATCH_9.md) 不新增数学定理；它记录 Pavlov--Scholbach modern colored admissibility/rectification、symmetric powers 技术条件和 Lurie associative/commutative strict-to-infinity algebra comparison 的精确定位。
...
- 第一章的对称序列、代入乘积、operad、endomorphism operad 和 Ass/Com 例子，主要依赖 Markl-Shnider-Stasheff 与 Loday-Vallette 的基础定义，并采用有限集分块口径重写。
...
- 第八章的二次 operad、Ginzburg-Kapranov 对偶和 Koszul 性，主要依赖 Ginzburg-Kapranov、Loday-Vallette 和 Fresse；classical Koszul core 已由 GK-1--GK-5 定位，现代符号版本按 [FINAL_OPERAD_THEORY_CLOSURE.md](FINAL_OPERAD_THEORY_CLOSURE.md) 作为 convention translation 处理。
- 第九章的 dg-operad、cooperad、bar-cobar 构造和 twisting morphism，主要依赖 Ginzburg-Kapranov、Loday-Vallette 和 Fresse；Ginzburg-Kapranov dg-dual core 已由 GK-6--GK-7 定位，Fresse modern cobar/twisted-composite 输入已由 FRE-1--FRE-4 定位，书本紧凑表述按最终收口文件处理。
...
- 第十四章的模型范畴中 operad、admissibility、rectification 和 Boardman-Vogt resolution，主要依赖 Berger-Moerdijk、Hinich、Fresse、Pavlov-Scholbach；Berger--Moerdijk/Hinich/Fresse 基础 locator 已由批次 1/5 定位，Pavlov--Scholbach modern colored admissibility/rectification 已由 PSAR-1--PSAR-6 与 PSP-1--PSP-2 定位，最终版仍需逐底范畴假设翻译。
- 第十五章的 simplicial/topological operads、Sing-realization 比较和 little cubes operad，主要依赖 Quillen、Goerss-Jardine、Hovey、May、Boardman-Vogt、Berger-Moerdijk 和 Fresse；Kan-Quillen 模型结构、Top 与 sSet 的 Quillen equivalence、operad 模型结构提升和 May recognition principle 均作为外部输入。
...
- 第十九章的 Dwyer-Kan localization、模型范畴的 underlying infinity-category、straightening/unstraightening、monoidal localization 和 operadic localization，主要依赖 Dwyer-Kan、Barwick-Kan、Hovey、Hirschhorn、Lurie、Hinich、Pavlov-Scholbach、White 和 White--Yau；ordinary straightening 已由 HTT-1 定位，Bousfield localization preservation 已由 WHT-1--WHT-4 与 WY-1--WY-3 定位，Dwyer--Kan/coherent nerve 已由 DKR-1--DKR-7 定位，strict-to-infinity algebra comparison 已由 PSAR-5--PSAR-6 与 HA-ALG-1--HA-ALG-3 定位，operadic straightening 已由 PRA-1--PRA-5 作最新/preprint locator 定位。
- 第二十章的 factorization algebras、factorization homology、Dunn additivity、Fukaya categories 和几何 gluing，主要依赖 Costello-Gwilliam、Ayala-Francis、Lurie、Seidel、Fukaya-Oh-Ohta-Ono 和 Ganatra-Pardon-Shende；excision/圆周/边界基础版本已由 AF-0--AF-4 定位，Dunn additivity 已由 DUNN-1 定位，locally constant factorization algebra、Fukaya category 构造和几何 gluing 定理仍作为外部几何边界。
...
- 附录 G 的模型结构假设、admissibility 和 rectification 检查表主要依赖 Berger-Moerdijk、Hinich、Fresse、Pavlov-Scholbach、Hovey、Hirschhorn 和 Lurie；operad transferred model structure、colored admissibility 和 rectification schema 已有 BM/HIN/FRE/PSAR locator，正特征反例边界仍需按具体模型说明。
...
- 附录 I 的 convolution Lie algebra、twisting morphism、twisted composite products、Koszul complex 和权重滤过主要依赖 Ginzburg-Kapranov、Loday-Vallette 和 Fresse；Koszul complex classical 判别已由 GK-3 定位，Fresse twisting acyclicity 已由 FRE-2--FRE-3 定位，现代 $\Omega\mathcal P^¡$ convention translation 按最终收口文件处理。
...
- 附录 M 的 strict/dendroidal/Lurie/model category 比较图主要依赖 Moerdijk-Weiss、Cisinski-Moerdijk、Heuts-Hinich-Moerdijk、Lurie、Hinich、Pavlov-Scholbach、White 和 White--Yau；White/White--Yau 支撑模型范畴 localization preservation，HHM-1--HHM-5 支撑 dendroidal-Lurie comparison，HA-OP-1--HA-OP-3 支撑 category-of-operators entry，PSAR/HA-ALG 支撑相应 strict-to-infinity algebra comparison。
- 附录 N 的 factorization homology、excision、圆周计算、切结构和 Fukaya gluing 边界主要依赖 Costello-Gwilliam、Ayala-Francis、Lurie、Seidel、Fukaya-Oh-Ohta-Ono 和 Ganatra-Pardon-Shende；excision、圆周 Hochschild 识别、boundary topological manifold 版本和 Dunn additivity 已定位，locally constant factorization algebra 等价与 Fukaya 型 gluing 仍作为外部几何边界。
...
- 附录 Q 的 Koszul complex、bar-cobar differential 和低权重计算主要依赖定义 8.4--定义 8.16、定义 9.14--定理 9.20 和定义 I.11--命题 I.21；bar-cobar resolution 的 modern cobar/cofibrant replacement 入口已由 FRE-4--FRE-5 定位，完整 Koszul 性和书本 convention translation 按最终收口文件处理。
...
- 附录 V 的带边界与分层 factorization homology 样例主要依赖定义 20.3--外部输入定理 20.23、定义 N.3--外部输入定理 N.30、错误命题 O.23--正确边界 O.28 和附录 R 的 rectification 案例；stratified factorization homology、sectorial descent 和 Fukaya skeletal descent 均作为外部输入。
