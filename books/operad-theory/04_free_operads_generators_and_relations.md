...
给定一个二元符号 $m$，表达式 $m(m(x_1,x_2),x_3)$ 与 $m(x_1,m(x_2,x_3))$ 在施加结合律以前应当是两个不同对象。平面有根树恰好区分这两种括号，而叶标号又记录对称输入的名字；把树 graft 到叶上则得到表达式代入。由此可以先构造不含任何关系的自由 operad，再取对重标号和所有代入都封闭的最小等价关系。这个过程把“生成元与关系”从公式速记变成泛性质明确的商构造，只需要前三章的 operad、树代入和商集语言。
...
装饰有根树给出了自由 operad 的具体元素，grafting 给出代入，树映射的泛性质则保证从生成元指定的函数唯一延拓为 operad morphism。关系不能只在某个 arity 中孤立取商；它必须在重标号与任意上下文代入下保持，因而要生成 operadic congruence。$\operatorname{Ass}$ 与 $\operatorname{Com}$ 的表示说明，熟悉的结合律、单位律和交换律正是这种稳定关系。第五章将让输入和输出携带类型，届时“哪些树可以 graft”还会受颜色匹配约束。
