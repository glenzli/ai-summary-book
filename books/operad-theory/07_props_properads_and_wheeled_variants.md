...
Operad 编码“多输入、单输出”运算。许多代数结构还需要多输出运算，例如余代数、双代数、Frobenius 代数和 trace 型结构。本章引入三类扩展：

1. PROP：允许多输入、多输出，并允许并排张量。
...
需要熟悉对称幺半范畴、operad、colored operad、张量积和有限有向图。
...
PROP 是有对象 $\mathbb N$ 的严格对称幺半范畴，编码多输入多输出运算及其并排张量。Properad 保留多输入多输出和图复合，但只允许连通图作为基本复合；PROP 可看作 properad 加上不连通水平张量。Wheeled 变体允许输出接回输入，适合 trace 和环路结构，但需要额外有限性或刚性假设。
