...
本文档记录《Langlands 纲领》审定前闭合版的概念层审定结论。它不新增数学内容，只规定全书核心概念的审定前统一口径，防止出版前审定时把口号、类比、猜想、外部输入和已证命题混用；它不是最终出版审定结论。
...
- 在第二十一章的函子方向约定下，Hecke eigensheaf 是 spectral 点对象在逆等价 $\mathbb L_G^{-1}$ 下的像。
...
本书已经达到概念审定前闭合：核心词语均已分层，四条主线的对象边界清楚，猜想、外部输入、外部输入的证明路线和本书内证明的状态可追踪。后续工作应是出版前审校，而不是概念体系重建；出版审校完成前，不把本结论表述为最终出版审定。
