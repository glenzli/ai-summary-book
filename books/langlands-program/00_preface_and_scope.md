...
Langlands 纲领常被压缩成一句“Galois 表示对应自守表示”，但这句话把真正的数学困难全部藏了起来：两侧的对象依赖局部域还是整体域，参数落在 Weil 群还是 L 群，所谓“对应”是双射、packet、函子还是范畴等价，而 L 函数相容又取决于一整套 Frobenius、测度和归一化约定。序章先把这些层次分开，并说明后文哪些构造会在正文中证明，哪些深定理将以外部输入的形式使用。技术定义从第一章开始；这里要建立的是一张不会混淆对象类型的阅读坐标图。
