...
费马大定理并不是完整 Langlands 纲领的直接推论，而是 `GL(2)/\mathbb Q` 模性理论的一条精确应用链。假定存在费马方程的反例，Frey 曲线把它变成一条具有特殊判别式和局部分歧的半稳定椭圆曲线；模性定理把其 Galois 表示送到权 $2$ newform，Ribet 降层再把残余表示压到级 $2$，而该级别的尖点形式空间为零。本章逐项陈述这条反证需要的外部输入，使最终矛盾不依赖含混的“由模性可得”口号。

椭圆曲线、模形式、Galois 表示和导子的系统定义见第六至十章。模曲线上同调、Taylor--Wiles 接口与 Frey 曲线局部计算分别见附录 T、U、AD；这些深层结果不在本章重证。Frobenius trace、残余导子与模形式 L 函数的转换遵循 [NORMALIZATION_TABLE.md](NORMALIZATION_TABLE.md) 第 5、6、7、8 节。
...
**费马应用的外部输入 90.A.** 本章证明只需要以下有限条结果，不需要完整的数域 `GL(n)` Langlands：
