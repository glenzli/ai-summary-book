...
任何光滑势在稳定平衡点附近的首个非平凡近似都是二次势，因此谐振子既是独立可解模型，也是分子振动、场模和小振幅运动的局部语言。直接求解二阶本征方程当然可行，但它掩盖了等间距能级的来源。把位置与动量组合成 $a$、$a^*$ 后，Hamiltonian 变为数算符的仿射函数，谱问题被改写为“每次升降一个量子”的代数问题。

无界升降算符也最容易诱发定义域错误。本章从第三章已在 Schwartz 空间上验证的 $[X,P]=iI$ 出发，先给出 $a$、$a^*$ 的闭算子定义域与共同核心，再在该核心上完成交换子计算。Gaussian 基态和 Hermite 数态提供显式波函数，相干态则检验无穷级数是否落在 $a$ 的定义域中。最后借外部输入的 Hermite 完备性补足全谱，并在书内推出各算子的加权平方可和定义域。
...
这一恒等式在共同核心上解释了代数机制，却还不能单凭形式交换子断言完整谱。下一步先把 $a^*a$ 定义为自伴非负数算符，并清楚说明升降操作在哪些向量上保持定义。
...
表达式。完整的 Hermite 族构造完成后，本章再调用其完备性外部输入，
并由加权移位计算得到 $a,a^*,N,H$ 的精确定义域和共同不变核心。
...
量子化论证说明共同核心中的本征值只能是非负整数；要确认每个整数确实出现，需要构造被 $a$ 消去的归一化基态，再反复施加 $a^*$。
...
数态给出离散能量的正交族，相干态则以所有数态的 Poisson 型叠加组成。
它们不是数算符本征态，却是湮灭算符本征态，因此把量子涨落与近经典
相空间运动联系起来。
...
$L^2(\mathbb R)$ 的正交归一基。这里采用 Teschl,
...
谐振子的等间距能级并非只来自一次漂亮的形式分解：共同核心保证交换子
计算合法，Gaussian 解给出升降链的起点，Hermite 完备性排除链外谱，
加权系数条件则恢复闭算子的精确定义域。这个模型以后会反复出现于
Landau 能级、场模和半经典波包；下一章先离开单系统，考察两个 Hilbert
空间组合后为何会出现不能还原为各自状态的纠缠。
