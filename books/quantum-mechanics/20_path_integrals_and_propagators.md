...
Schrodinger 图像把演化写成 Hilbert 空间中的酉算子；位置表象则希望用
一个核 $K(t;x,y)$ 回答“从 $y$ 到 $x$ 的振幅如何传播”。自由粒子的核
振幅模长不随 $|x-y|$ 衰减，普通 Lebesgue 积分并不绝对收敛，所以它首先
是振荡积分或 tempered distribution。形式路径积分把传播再拆成许多短时
步骤，但无穷维路径空间上并不存在可直接套用的平移不变 Lebesgue 测度。

本章先由算子核定义传播子，并在可用 Fubini 或分布核复合的条件下证明
卷积律。Fourier 变换随后精确算出自由核及其 Fresnel 分支。加入势能时，
Lie--Trotter 公式作为外部输入保证交替乘积的强收敛；单步 split 核则在
书内精确计算。这样，离散作用量如何出现可以逐项追踪，同时不会把单步
形式核误称为一般 Hamiltonian 的逐点误差展开。
...
卷积律说明中间位置积分只是算子复合的核表示。对自由 Hamiltonian，
Fourier 表象中的演化是一个二次相位乘法，因而可以把核完整反变换出来。
...
自由核中的二次相位正是离散动能作用量。加入位置势以后，动能与势能
指数通常不交换；要把短时间切片变成严格极限，需要乘积公式而不是形式
Taylor 展开。
...
Lie--Trotter 定理控制整个 $n$ 步算子乘积的强极限。为看清每一步实际
积分的对象，下面先计算固定非零步长下 split 算子的精确分布核。
...
传播子把酉群复合律变成核的卷积，自由粒子则给出可显式检验的 Fresnel
振荡核。对动能加势能，单步 split 核是精确的，但它与完整短时演化的
关系只由满足假设的 Lie--Trotter 强极限保证。形式路径积分因此是离散
作用量极限的组织语言，而不是未经构造的实时间概率测度。下一章恢复
$\hbar$ 与 $c$，检验把非相对论一粒子框架推向相对论时哪些概率解释会
失效。
