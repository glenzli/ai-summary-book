...
图像解满足命题 25.2 的强可微性与定义域假设，则相应相互作用图像态
满足
$$
i\dot\psi_I(t)=\lambda V_I(t)\psi_I(t).
$$
本节实际把演化定义为这个有界方程的积分解。在上述范数连续假设下，
积分方程使用算子范数 Bochner 积分，其唯一传播子和 Dyson 级数由
命题 25.4 给出（本章取其中 $t_0=0$ 的情形）。
仅假设强连续和一致有界时可以
...
$$
命题 25.4 在 $t_0=0$ 时保证该积分方程及其 Dyson 展开按算子范数
成立。一阶截断把积分中的 $\psi_I(s)$ 替换为初态 $i$；再取 $f$ 分量
...
**证明.** 由命题 25.4，第 $r$ 阶时间有序 Bochner 积分的算子范数
至多为
$$
