...
| QM-EXT-7 | 绝热定理 | 第 16 章以一致谱隙控制离开瞬时谱子空间的 $O(\varepsilon)$ 范数误差 |
...
| QM-EXT-10 | 球谐函数完备性与 Coulomb 谱理论 | 第 22 章角向完备性、Coulomb 自伴实现和氢原子完整能谱；基态微分方程在书内核对 |
...
- 第 8 章的可分性、秩一约化态与纯度判别由 Schmidt 系数在章内直接证明，不依赖第 17 章的纯态判别。
- 第 10--11 章分别依赖 QM-EXT-5 与 QM-EXT-4。
- 第 12 章的有限维简单本征分支由实解析隐函数定理在书内构造；范数连续有界相互作用的 Picard 解、Dyson 级数、酉性、复合律和有限时间余项也在章内闭合。QM-EXT-20 只承担无限维解析扰动边界。
- 第 13--14、16、18、20 章各自依赖 WKB 连接、散射、绝热、一般完全正映射表示和 Trotter 相关外部输入；有限维 Naimark 扩张在第 18 章书内证明。
- 第 19 章的等先验二元态区分公式由差算子的正负谱分解在书内证明；Uhlmann 型保真度结构仍使用 QM-EXT-19。
- 第 22--23 章依赖球谐完备性、Coulomb Hamiltonian 谱理论、磁 Schrodinger 算子的自伴性和 Landau 纤维中的 Hermite 完备性。
- 第 24 章的 Robertson、连续性方程与 Ehrenfest 公式在声明的共同定义域或正则性假设下内部闭合。
- 第 25 章从相互作用图像方程重新构造二参数 Dyson 传播子，不反向调用第 12 章；命题 25.4 的存在唯一性先于命题 25.4A 的酉性与复合律，再推出命题 25.5 的跃迁振幅。连续谱黄金规则的严格弱耦合极限仍依赖谱测度和散射理论。
...
| 章节 | 书内依赖顺序 | 外部边界 |
|---|---|---|
| 第 12 章 | 设定 12.4 $\to$ 命题 12.4A 的 Picard/Dyson 构造 $\to$ 命题 12.4B 的酉性与复合律 $\to$ 命题 12.5 的一级振幅；命题 12.5A 独立由 12.4A 的单纯形估计给余项 | 无界或仅强连续相互作用不由该范数级数覆盖；无限维定态谱分支使用 QM-EXT-20 |
| 第 25 章 | 命题 25.2 的相互作用图像方程 $\to$ 命题 25.4 的唯一 Dyson 解 $\to$ 命题 25.4A 的二参数传播子结构 $\to$ 命题 25.5 的跃迁振幅 $\to$ 命题 25.6 的分布极限 | 黄金规则的连续谱归一化、阈值与严格弱耦合极限使用 QM-EXT-8 |

两条链分别在各章从积分方程开始构造，不以另一章中尚未建立的结果作为
证明前提。全书编号定理的正文引用均位于其陈述之后。
