...
**答案 12.3.** 因 $V_I(r)=V_0$，第 $n$ 阶 Dyson 项为
$$
(-i\lambda)^n
\int_{s\le r_n\le\cdots\le r_1\le t}
V_0^n\,dr_1\cdots dr_n
=\frac{[-i\lambda(t-s)V_0]^n}{n!}.
$$
对 $n\ge0$ 求和得到
$$
U_I(t,s)=e^{-i\lambda(t-s)V_0}.
$$
$V_0$ 自伴且 $\lambda(t-s)$ 为实数，所以该指数是酉算子，也可直接
验证它满足积分方程与初值 $U_I(s,s)=I$。
...
**答案 18.3.** 因
$r_z=\operatorname{tr}(\rho\sigma_z)=p-(1-p)=2p-1$，
$$
p_\pm=\frac12\left(1\pm\eta(2p-1)\right).
$$
$\eta=0$ 时 $p_+=p_-=1/2$，结果与输入态无关；$\eta=1$ 时
$p_+=p$、$p_-=1-p$，恢复计算基中的锐利投影测量。
...
**答案 19.3.** 两个相同态满足 $D(\rho,\rho)=0$，故
$$
p_{\mathrm{succ}}^{\max}=\frac12.
$$
两个正交纯态的内积为零，由命题 19.10 得 $D=1$，所以
$$
p_{\mathrm{succ}}^{\max}=1.
$$
后一情形可由投影到两条正交射线的测量完全区分。
