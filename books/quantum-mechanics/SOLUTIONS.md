...
$$
故一阶修正为 $0$；实际能级移动从二阶开始。

**答案 12.2.** 简并时 $E_n^{(0)}-E_m^{(0)}=0$ 可能出现在分母中，非简并公式无定义。必须先在简并子空间中对角化 $V$，得到正确的零阶基和一阶能量。
...
**答案 13.2.** 无限深方势阱中 $p=\sqrt{2mE}$，转折点为 $0,L$。WKB 条件给出
$$
L\sqrt{2mE}\approx \pi\hbar n,
$$
所以
$$
E_n\approx\frac{\pi^2\hbar^2n^2}{2mL^2},
$$
与精确能级主项一致。边界相位的精细处理决定 $n$ 与 $n+1/2$ 的差别。
...
因此 Dyson 级数化为
$$
\exp\left(-i\int_{t_0}^tV_I(s)\,ds\right).
$$
