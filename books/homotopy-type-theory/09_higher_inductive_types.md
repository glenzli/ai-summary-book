...
若把圆当作只有一个点构造子的普通归纳类型，它会与单位类型没有区别；真正区分圆的是一条指定环路。高阶归纳类型（higher inductive type, HIT）允许点、路径乃至更高路径同时成为构造子，但新增一条路径构造子还不够：必须说明依赖函数怎样从这些生成元上定义，以及消去项在每个维度上如何计算。

本章以圆为完整样本，逐项写出形成、构造、依赖消去和计算规则，并比较点计算与路径计算的强度。截断和商已经在第八章先行作为附录 L 的规则包使用；这里解释那种输入应具有什么形状。一般 HIT 的存在性不是对象语言定理，更一般的 HIIT、QIT 与 QIIT 也需要各自的严格正性和模型论，因此只在精确外部边界内出现。
...
圆的例子表明，HIT 的数学内容不只在生成元列表中，而在依赖消去与高维计算的配合中。公理化规则包可以令点分支 judgmentally 计算、路径分支 propositionally 计算；特定 cubical 语法可能更强，但那是另一个归约系统的定理。后面使用悬挂与 pushout 时，都将沿用同一检查顺序：先固定宇宙和构造子，再给消去数据，最后核对每条计算规则的相等强度。
