...
本附录依赖附录 W 对 M.14 的整数加法群律证明核。换言之，附录 W 证明整数对象自身形成交换群；本附录证明附录 N 的 encode-decode 等价与群运算相容，并由此得到圆的基本群同构。附录 N.8 已按本书固定的圆规则核对计算强度：基点计算是 judgmental，loop 计算通过指定的 propositional $\beta$ 路径完成。
