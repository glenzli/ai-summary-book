...
4.  若后续扩写本书，应先为每个 HIT/QIIT 建立签名，再证明递归/消去原则与当前手写定义一致。

## BC.8 本附录关闭的缺口

本附录把“一般 HIT 元理论”拆为可审查项：签名、代数、初始性、消去原则、计算规则、cubical 语义和 universe 风险。剩余义务不是概念缺失，而是为每个具体签名给出语法、模型和 canonicity 证明。
