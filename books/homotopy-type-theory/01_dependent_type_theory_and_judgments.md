...
普通函数记号 $f(x)$ 隐藏了一个前提：$x$ 必须属于 $f$ 的定义域。依赖类型论把这个前提推进到每个表达式内部。例如在语境 $x:A$ 中，$B(x)$ 才是可用的类型；把 $a:A$ 替换给 $x$ 后，项 $b(x):B(x)$ 的类型也必须同时变成 $B(a)$。若只替换项而忘记替换类型，后面的 transport、fiber 和单值性公式都会在最基本的层面失去类型。

本章从这种“表达式总在语境中”的事实出发，建立类型、项、判断相等、替换以及 $\Pi$、$\Sigma$ 两种依赖构造。这里只需要普通函数和变量替换的经验；恒等类型、函数外延性与单值性尚未进入语言，因此本章中的相等只能来自语法规则和计算。
...
这一组规则已经能定义恒等函数、函数复合、依赖截面和总空间，却仍不能把两个仅仅逐点相同的函数判为相等。这个缺口不是遗漏：judgmental equality 负责计算，下一章的恒等类型则把“两个项相等”变成可以拥有不同证明的内部类型。二者的分离将贯穿全书。
