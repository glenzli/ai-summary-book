...
状态：已关闭为书内证明核。  
说明：路径归纳、$\Sigma$ 路径、fiber 收缩、等价复合、准逆相干化、等价保持 h-level 和 h-level 向上闭包均已有书内证明核。

**K.1.2 外延性、单值性与 SIP。**  
范围：第 6-8 章，附录 F、H、I、J、AG、T。  
状态：书内证明核 / 外部输入保留。  
说明：函数外延性、单值性、命题外延性和结构等同性原则均有明确引入位置。单值性推出函数外延性的长证明保留为精确外部输入或压缩证明路线，不阻塞文本封稿。

**K.1.3 HIT、截断、圆与基本群。**  
范围：第 8-11 章，附录 L、M、N、V、W、AD、AI。  
状态：已关闭为书内证明核与输入规则。  
说明：本书实际使用的命题截断、一般截断、集合商、圆、悬挂和 pushout 均以规则表给出；圆的 encode-decode、整数加法群律和 $\pi_1(\mathbb S^1)\cong\mathbb Z$ 有证明核。一般 HIT 元理论仍是第十六章外部元理论边界。
...
状态：外部输入 / 研究边界保留。  
说明：cubical type theory、simplicial model、Rezk/Segal types、QIIT、2LTT、directed/simplicial type theory、cohesive HoTT、SDG/SAG 和 displayed/bicategory coherence 均不得回流为 L0-L5 的隐式规则。
