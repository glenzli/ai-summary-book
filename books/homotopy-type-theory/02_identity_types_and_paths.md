...
第一章的 $a\equiv b:A$ 只能记录计算器已经把两个表达式认作同一项；它不能承载一个输入，也不能再被比较。数学中的相等却经常是数据：给定 $p:x=y$，我们要沿 $p$ 改变依赖项的类型；给定两条证明 $p,q:x=y$，还可能继续研究 $p=q$。恒等类型正是把这种数据放回对象语言的构造。

本章从四条规则建立 $\mathsf{Id}_A(a,b)$，然后反复使用同一个消去原则 $J$。transport、逆路径、复合、$\mathsf{ap}$ 与 $\mathsf{apd}$ 看似是不同操作，实际都由“先处理反身路径，再由路径归纳推广”生成。整个推导只依赖第一章的语境、替换、$\Pi$、$\Sigma$ 与判断相等，不会预借函数外延性或单值性。
...
从一个构造子 $\mathsf{refl}$ 和一个消去原则 $J$ 出发，我们已经得到 transport、$\mathsf{apd}$、逆路径、复合与群胚律。关键不是把路径想象成几何曲线，而是每次都写出 motive，并检查反身分支的类型。所得结合律、逆律和自然性都是恒等类型中的项；除规则明确给出的 $\beta$ 情形外，它们不会升级为 judgmental equality。下一章的归纳类型将提供具体数据，而本章的路径代数负责比较这些数据。
