...
Einstein 方程是非线性的，但远离弱源时，度规可看作 Minkowski 背景上的小扰动。困难在于 $h_{\mu\nu}$ 同时含物理传播自由度和坐标规范自由；若不先控制展开阶数与规范，十个分量不会自动等于十种引力波。本章使用第六章的曲率约定、第七章的测地线偏离和第八章的场方程，推导 Lorenz 规范下的线性方程及两个 TT 偏振，并把四极矩辐射与 Isaacson 能流的额外近似明确列出。全章取 $(-,+,+,+)$、$c=1$、$\Lambda=0$，并约定 $\Box=-\partial_t^2+\nabla^2$；非零 $\Lambda$ 时必须改在相应曲背景上展开。
