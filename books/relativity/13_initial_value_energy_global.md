...
局部写下 Einstein 方程并不等于已经建立一个可预测的动力学理论。给定空间超曲面上的度规和外曲率，哪些数据允许演化，解是否存在且在微分同胚意义下唯一，又能延拓多远，都是整体问题。同时，弯曲时空中的“总能量”只有在适当渐近条件下才获得不依赖坐标的定义。本章从 ADM 约束出发，区分书内可完成的投影计算与最大 Cauchy 发展、正质量和奇点定理等外部输入，并用全局双曲性说明初值面为何能够控制因果演化。
