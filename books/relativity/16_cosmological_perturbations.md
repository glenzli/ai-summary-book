...
扰动分量本身会随坐标微扰而改变，因此第一项任务不是求解方程，而是分清规范伪影与可比较的物理组合。以下借助第十一章的 FLRW 方程和第十二章的线性规范思想，构造 Bardeen 势，并只在无压物质、亚视界和 Newton 近似同时成立时推导密度增长方程。撇号表示共形时间 $\eta$ 导数，圆点仅在明确声明后表示固有时 $t$ 导数，背景先取空间平直。Boltzmann 多组分演化、原初量子涨落与非线性结构形成作为精确标界的后续理论，不混入书内已完成的流体推导。
