...
这里尤其要区分三个层次：$\Delta=0$ 是坐标表达中的代数条件，Killing 视界是局部对称结构，事件视界则依赖完整未来因果结构。第六章的 Killing 场、第七章的仿射测地线、第十章的全局视界定义和第十三章的能量条件共同提供所需语言。显式 Killing 守恒量将在书内推导；Carter 可分离性、最大延拓和面积定理则以列全假设的外部输入出现。
