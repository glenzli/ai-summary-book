...
电场与磁场会在 Lorentz boost 下彼此混合，这表明它们不是两种独立的相对论对象。真正保持协变的是反对称场强张量 $F_{\mu\nu}$；三维 Maxwell 方程、Lorentz 力、电荷守恒和规范自由都应从它及四势的结构中恢复。这里沿用第一章的四矢量与张量语言，取 $x^0=t$、$(-,+,+,+)$ 号差、$\epsilon^{123}=+1$、$\epsilon^{0123}=+1$ 和 Heaviside--Lorentz 单位。固定这些约定不是形式工作，因为指标次序或取向改变会同时翻转分量公式与波动方程中的符号。
