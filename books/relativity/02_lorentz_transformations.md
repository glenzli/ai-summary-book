...
本章在 $c=1$、度规号差 $(-,+,+,+)$ 的约定下推导标准 boost 的几何与测量后果，并把因果次序、快速度和双生子效应写成带明确假设的命题。

## 依赖前置知识

需要第一章的 Minkowski 时空、固有时、时间定向和 Lorentz 变换定义。本章只讨论 proper orthochronous Lorentz 变换，除非另行说明。
