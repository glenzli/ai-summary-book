...
Minkowski 几何规定了因果结构，却还没有告诉我们能量和动量应如何组合。若继续把三动量与能量视为互不相关的量，碰撞阈值、无质量粒子和连续介质的守恒律都会依赖所选惯性系。解决办法是从四速度构造四动量，并把单粒子的守恒推广为场和流体的应力能量张量。本章沿这条路径从质量壳走向完美流体，同时用第零章的作用量变分和前两章的 Lorentz 几何核对每个对象的类型、符号与不变量。
