...
粒子方程、场方程与守恒律若分别作为经验公式列出，它们之间的共同结构仍然不可见。作用量把三者放进同一个变分问题：轨道和场是驻定点，连续对称性则产生守恒流。这个统一只有在允许的变分和边界条件写清之后才成立。以下使用第零章的变分基本引理、第三章的四动量和第四章的电磁势；世界线变分固定端点，场变分取紧支撑或在边界上消失。由此可以逐步推导自由粒子、带电粒子与实标量场，并为曲时空中的能动张量定义准备接口。
