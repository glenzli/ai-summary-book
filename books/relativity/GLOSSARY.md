...
**ADM 能量**：选定渐近平直端和渐近观察者后，由空间度规的一阶渐近量
定义的边界积分 $E_{\mathrm{ADM}}$。它是 ADM 四动量的时间分量，会在
渐近 boost 下改变。

**ADM 动量**：由外曲率渐近量定义的边界积分
$P_i=P_i^{\mathrm{ADM}}$；它与 $E_{\mathrm{ADM}}$ 组成 ADM 四动量。

**ADM 质量**：ADM 四动量未来非类空时的 Lorentz 不变量
$m_{\mathrm{ADM}}=\sqrt{E_{\mathrm{ADM}}^2-|\mathbf P|^2}$。若四动量
类时，它等于渐近静止系中的 ADM 能量；一般渐近系中的
$E_{\mathrm{ADM}}$ 不能直接称为不变质量。
...
动量密度、能流与应力，并作为 Einstein 方程的源。第五章另以
$\Theta^\mu{}_\nu$ 表示规范张量，并固定其号差使最小耦合实标量场在
平直背景上满足 $\Theta^\mu{}_\nu=T^\mu{}_\nu$。
