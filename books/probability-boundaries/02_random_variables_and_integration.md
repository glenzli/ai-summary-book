...
**定义 2.1（随机元素）.** 设 $(\Omega,\mathcal F,\mathbb P)$ 是概率空间，$(S,\mathcal S)$ 是可测空间。可测映射
...
**命题 2.4（分布函数的基本性质）.** 对每个实随机变量 $X$，$F_X$ 单调不减、右连续，并满足
...
**定义 2.5（密度）.** 设 $(S,\mathcal S,\lambda)$ 是 $\sigma$-有限测度空间。若 $\mathbb P_X\ll\lambda$，则由 Radon--Nikodym 定理存在 $\mathcal S$-可测函数 $p:S\to[0,\infty]$，使
...
**外部输入 2.6（Lebesgue 积分）.** 对测度空间 $(S,\mathcal S,\mu)$，存在非负可测函数的积分 $\int f\,d\mu\in[0,\infty]$，满足单调性、简单函数逼近、单调收敛定理和 Fatou 引理。对实函数 $f=f^+-f^-$，若至少一个 $\int f^+d\mu,\int f^-d\mu$ 有限，则可定义扩展积分；若二者都有限，则称 $f$ 可积。支配收敛定理将在第五章以所需版本重述。该构造见 [SOURCES.md](SOURCES.md) 中的标准测度论资料，本书不重建 Caratheodory 扩张与全部积分理论。

**定义 2.7（期望）.** 对实随机变量 $X$，若 $X$ 的扩展积分有定义，记
...
**定理 2.8（推前积分公式）.** 设 $X:(\Omega,\mathcal F)\to(S,\mathcal S)$ 是随机元素，$g:S\to\overline{\mathbb R}$ 可测。若 $g\ge0$，或 $g\circ X$ 可积，则
...
**定义 2.9（矩与方差）.** 对整数 $k\ge1$，若 $X^k\in L^1$，定义 $k$ 阶原点矩；若 $X\in L^2$，定义方差：
...
**定理 2.12（Markov 不等式）.** 若 $X:\Omega\to[0,\infty]$ 可测且 $a>0$，则
...
**推论 2.13（Chebyshev 不等式）.** 若 $X\in L^2$，则对每个 $\varepsilon>0$，
...
**定理 2.14（有限 Jensen 不等式）.** 设 $I\subseteq\mathbb R$ 是区间，$\varphi:I\to\mathbb R$ 为凸函数，$x_1,\ldots,x_n\in I$，$\lambda_i\ge0$ 且 $\sum_i\lambda_i=1$。则
...
**练习 2.1.** 设 $X$ 在 $\{0,1,2\}$ 上的概率分别为 $1/4,1/2,1/4$。计算 $\mathbb EX$ 与 $\operatorname{Var}(X)$。

**练习 2.2.** 给出两个同分布但不几乎处处相等的随机变量。

**练习 2.3.** 证明若 $X=Y$ 几乎处处，则 $\mathcal L(X)=\mathcal L(Y)$；说明逆命题为何不成立。

**练习 2.4.** 用 Markov 不等式证明：若 $X\ge0$ 且 $\mathbb EX=0$，则 $X=0$ 几乎处处。

**练习 2.5.** 为两个 Bernoulli$(1/2)$ 边缘分布构造独立、完全相同和完全相反三种耦合。
