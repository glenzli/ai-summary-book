...
**定义 5.2（$L^p$ 收敛）.** 对 $p\ge1$，若每个 $X_n,X\in L^p$ 且
...
**定义 5.3（依概率收敛）.** 若对每个 $\varepsilon>0$，
...
**定义 5.4（依分布收敛）.** 若对每个有界连续函数 $f:\mathbb R\to\mathbb R$，
...
**定理 5.5（$L^p$ 收敛蕴含依概率收敛）.** 若 $X_n\to X$ in $L^p$，$p\ge1$，则 $X_n\xrightarrow{\mathbb P}X$。
...
**定理 5.6（几乎处处收敛蕴含依概率收敛）.** 若 $X_n\to X$ a.s.，则 $X_n\xrightarrow{\mathbb P}X$。
...
**外部输入 5.7（依概率收敛蕴含依分布收敛）.** 若 $X_n\xrightarrow{\mathbb P}X$，则 $X_n\Rightarrow X$。这是 Portmanteau 理论的标准推论。反向一般不成立；若 $X_n$ 与 $X\equiv c$ 定义在同一概率空间上，则 $X_n\Rightarrow c$ 等价于 $X_n\xrightarrow{\mathbb P}c$。所用版本与来源见 [SOURCES.md](SOURCES.md) 的弱收敛条目。

**定理 5.8（依概率收敛的几乎处处收敛子列）.** 若 $X_n\xrightarrow{\mathbb P}X$，则存在严格递增的指标序列 $(n_k)$，使 $X_{n_k}\to X$ 几乎处处。
...
**定义 5.9（一致可积）.** 一族可积随机变量 $\mathcal X$ 称为一致可积，若
...
**外部输入 5.10（支配收敛与 Vitali 收敛）.** 本书使用以下两个标准版本：
...
第一项对 $|X_n-X|^p$ 应用 Lebesgue 支配收敛定理；第二项是 Vitali 收敛定理。完整证明依赖积分的绝对连续性与截断论证，作为测度论外部输入，来源见 [SOURCES.md](SOURCES.md)。
...
几乎处处收敛与 $L^p$ 收敛彼此一般不可比较；依概率收敛也不保证 $L^1$ 收敛。定理 5.8 只保证可选出几乎处处收敛的子列，不把原序列升级为几乎处处收敛。
...
**定理 5.11（弱大数律，i.i.d. 有限方差版本）.** 若 $X_1,X_2,\ldots$ 独立同分布且 $\operatorname{Var}(X_1)=\sigma^2<\infty$，令 $\mu=\mathbb E X_1$。则
...
**外部输入 5.12（Kolmogorov 强大数律，i.i.d. 可积版本）.** 若 $X_1,X_2,\ldots$ 独立同分布且 $\mathbb E|X_1|<\infty$，则

$$
\overline X_n\to\mathbb EX_1\quad\text{几乎处处}.
$$

本书使用该结论说明长期频率的路径级收敛，不重证其截断、最大不等式与 Borel--Cantelli 论证。来源见 [SOURCES.md](SOURCES.md)。

**外部输入 5.13（Lindeberg--Levy 中心极限定理）.** 若 $X_1,X_2,\ldots$ 独立同分布，$\mathbb EX_1=\mu$，且 $0<\operatorname{Var}(X_1)=\sigma^2<\infty$，则
...
该定理给出归一化误差的极限分布，不说有限 $n$ 时误差恰好服从正态分布。若方差为零，标准化无定义且样本均值恒等于 $\mu$；若方差无限，该版本不适用。所用 i.i.d. 版本与来源见 [SOURCES.md](SOURCES.md)。

## 5.3 浓缩界与误用边界

Chebyshev 界只使用方差，通常较松。“变量有界时有指数尾界”需要独立性和明确常数，不能只靠有界性口号替代。

**外部输入 5.14（Hoeffding 不等式）.** 设 $X_1,\ldots,X_n$ 相互独立，并且存在实数 $a_i\le b_i$ 使
...
**练习 5.1.** 构造依概率收敛但不几乎处处收敛的随机变量序列。

**练习 5.2.** 构造 $X_n\to0$ 依概率成立但 $\mathbb E|X_n|\not\to0$ 的例子。

**练习 5.3.** 用弱大数律给出公平硬币正面频率偏离 $1/2$ 至少 $0.1$ 的 Chebyshev 上界。

**练习 5.4.** 说明中心极限定理为何不蕴含 $\overline X_n$ 自身收敛到正态分布。

**练习 5.5.** 列出把一个固定基准集分数解释为总体性能估计时所需的抽样与选择假设。

**练习 5.6.** 对 i.i.d. Bernoulli$(1/2)$ 变量，分别用 Chebyshev 与 Hoeffding 不等式界定 $\mathbb P(|\overline X_n-1/2|\ge\varepsilon)$，比较两者随 $n$ 的衰减速度。
