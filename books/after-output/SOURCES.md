...
七部既有书稿曾作为编辑输入，但不是本书主张的外部证据。正文和附录使用的论文、规范与专著均在本文件直接登记；不得以“原书写过”为事实来源或证明边界。各部分的内容承接见 `MERGE_MAP.md`，原目录能否从工作树移除见 `SOURCE_RETIREMENT_AUDIT.md`。即使原目录退役，本文件、主张表、附录和 Git 历史也必须使第三方能够区分书内证明、外部输入与有意舍弃的重复材料。
