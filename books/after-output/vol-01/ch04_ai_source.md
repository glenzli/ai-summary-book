...
ELMo、BERT、GPT、T5/BART 代表不同接口：

| 模型族 | 结构口径 | 训练目标 | 输出接口 |
| --- | --- | --- | --- |
| ELMo | 双向语言模型特征 | 语言建模 | 下游特征 |
| BERT | encoder-only | MLM/NSP 等 | 表示、分类、抽取 |
| GPT | decoder-only | next-token prediction | 自回归生成 |
| T5/BART | encoder-decoder | text-to-text / denoising | 条件生成 |

这里关心的不是谁更先进，而是接口差异。encoder 表示不直接构成生成轨迹；decoder 输出天然形成 token 前缀；text-to-text 把任务统一为文本接口，却不消除不同任务的证据差异。

### S2.5 预训练与基座模型


预训练通过大规模数据和自监督目标获得通用表示或生成能力。基座模型可能编码大量语言和世界模式，但它还不是完整助手：稳定指令接口、拒答边界、工具协议、对话状态和组织责任都属于后续层。

因此，“模型能生成相关 token”与“系统能可靠完成任务”之间至少隔着上下文组装、外部事实接入、权限控制和提交核验四类接口。
