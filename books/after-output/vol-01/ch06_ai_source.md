...
下一步的关键不只是增加模态，还包括跨模态对齐、时间推理、证据校准和行动反馈。相关延伸见 **[6.6 世界模型与生成式媒体](ch06_ai_source.md#section-6-6)**。
<a id="section-6-2"></a>
...
这套最小闭环解释了语言模型怎样从回答者变成控制器，却还没有解决协议发现、持久状态、跨 Agent 协作和安全审计。关于 2026 年前后更完整的 Agent 运行时、MCP/A2A 协议、上下文工程、多 Agent 编排、权限安全与轨迹评测，见 **[6.5 Agent 系统工程](ch06_ai_source.md#section-6-5)**。
<a id="section-6-3"></a>
...
本节补上了“世界模型”和“生成式媒体”的主线。回看 **[6.5 Agent 系统工程](ch06_ai_source.md#section-6-5)** 时，读者应区分两点：许多工具型 Agent 只依靠语言模型、状态机和环境反馈即可运行，并未显式训练世界模型；需要长期规划或物理控制时，能够预测行动后果的显式或隐式动力学表示会更重要。视频生成、语言预测表示与控制世界模型之间有研究联系，但不能互相等同。

> **本章收束**：
> 本章把多模态、Agent、记忆、协议和世界模型放回系统层。全书到这里收束为一条主线：模型能力来自表示、训练、后训练与运行时系统的组合，任何产品或榜单条目都应回到机制、证据和时效边界中阅读。
...
自回归、潜变量、扩散、跨模态对齐和世界模型扩展了可生成对象，却没有消除系统边界。相反，它们引入更多编码器、采样器、制品格式、来源关系和闭环状态。下一章将把这些模型工件放入完整部署系统，说明模型能力何时才成为一次可归因的系统行为。
