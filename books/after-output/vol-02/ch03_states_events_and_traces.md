...
文本只是一种观察函数 $\pi(t)$ 的结果。两条轨迹可能观察文本相同，却有不同工具副作用。
...
其中 $S$ 为状态集合，$A$ 为事件标签，$\to\subseteq S\times A\times S$。轨迹是

$$
s_0\xrightarrow{a_0}s_1\xrightarrow{a_1}\cdots.
$$

常见事件包括：读取上下文、生成 token、发起工具请求、收到响应、提交流式片段、写入 artifact、取消、重试和失败。若转移关系对每个状态和标签唯一，才可谈确定性；若标签选择或外部返回未固定，则轨迹不唯一。
