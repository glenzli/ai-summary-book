...
设 $\mathbb B=\{0,\ldots,255\}$ 为字节集合，$\mathbb U$ 为 Unicode 标量集合，$\mathbb V$ 为固定 tokenizer 的 token ID 集合。最终消息可以有三种不同表示：

$$
b\in\mathbb B^*,\qquad u\in\mathbb U^*,\qquad v\in\mathbb V^*.
$$

UTF-8 解码、Unicode 规范化和 tokenizer 解码都需要版本与域。字节相同通常推出文本相同；文本相同不推出 token 相同，因为 tokenizer 版本和预处理可能变化；token 相同也不直接给出事实真值。

### S6.1 字节、Unicode 与 token 的合法域


设 $\mathbb B=\{0,\ldots,255\}$。UTF-8 解码不是从所有字节串到文本的总函数，而是部分函数

$$
\operatorname{Dec}_{utf8}:\operatorname{ValidUTF8}\subseteq\mathbb B^*
\to \mathbb U^*.
$$

宽容解码会把非法字节替换为特殊字符，但那是另一个函数。审计时不能把非法字节、宽容文本和原始输入混为一谈。

Tokenizer 也是带版本的制品：

$$
\operatorname{Tok}_V:\mathbb U^*\rightharpoonup \mathbb V^*,
\qquad
\operatorname{Detok}_V:\mathbb V^*\rightharpoonup \mathbb U^*.
$$

特殊 token、规范化、并列最长匹配和禁止 token 都必须由 $V$ 固定。同一 Unicode 文本在不同 tokenizer 下可能得到不同 token 序列。
