...
本章介绍用范畴论表达“组合结构”和“递归数据类型”的标准工具。多项式函子由一串映射 $I\leftarrow E\to B\to J$ 给出；species 描述带标号有限集合上的结构；解析函子由 species 的同伦商生成；W-types 是多项式函子的初代数。它们连接 combinatorics、类型论、operads、容器和高阶代数。

## 依赖前置知识

需要 slice categories、pullback、dependent sum/product、locally Cartesian closed categories、groupoids、Kan extensions、initial algebras、operads 和基本类型论。
...
**外部输入定理 52.10.** 在 elementary topos、locally Cartesian closed category 或 type-theoretic model 的合适假设下，某些多项式函子有 W-types。

**命题 52.11.** 自然数对象是多项式函子 $P(X)=1+X$ 的 W-type。
