...
把弱等价形式地取逆可以得到同伦范畴，却会丢失映射之间的高阶同伦。Quillen 模型结构在同一范畴上指定弱等价、纤维化和余纤维化三类态射，使替换、提升和分解能够计算正确的导出映射。余纤维--纤维对象之间的同伦类给出局部化，而 Quillen 伴随进一步控制导出函子。本章以模型公理为起点，证明同伦范畴的基本构造，并说明它为何只是后续 $\infty$-范畴的 1-截断影子。

所需背景是极限、余极限、局部化和基本同伦直觉。小对象论证等存在性机器不在本章无条件使用；凡涉及 cofibrant/fibrant replacement 的函子选择，都会注明所需分解公理与结论只在同伦意义下唯一。
