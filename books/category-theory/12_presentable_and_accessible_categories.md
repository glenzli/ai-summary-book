...
许多“大”范畴无法由全部对象同时操作，却可以由一族小对象经滤过余极限生成。正则基数 $\kappa$ 用来度量这种小性：$\kappa$-紧对象的 Hom 函子保持 $\kappa$-滤过余极限，可达范畴则由这些对象生成。若再有所有小余极限，便得到局部可表现范畴。这个框架把大小控制转化为可计算的生成定理，也是伴随函子定理、Bousfield 局部化和 presentable $\infty$-范畴的共同基础。

本章沿用 universe、小余极限与生成族的约定。正则性、滤过性和紧性中的量词会逐项展开；尤其不把“有限呈示”“紧”“可达”视为无条件同义词，而始终记录所选基数与范畴是否局部小。
...
**证明.** 设 $\mathcal G$ 为 $\kappa$-紧对象小骨架，并假设 $\mathcal C(G,f)$ 对所有 $G\in\mathcal G$ 都是双射。任取 $Z\in\mathcal C$。由局部 $\kappa$-可表现性，可写

$$
Z\cong\operatorname*{colim}_{i\in I}G_i,
$$

其中 $I$ 为小的 $\kappa$-滤过范畴且 $G_i\in\mathcal G$。Hom 函子把第一变量中的余极限变为极限，故

$$
\mathcal C(Z,f)\cong
\lim_{i\in I^{op}}\mathcal C(G_i,f)
$$

也是双射。于是自然变换 $\mathcal C(-,f)$ 在每个对象上均为双射；由 Yoneda 引理，$f$ 是同构。反向显然。$\square$
...
有右伴随，当且仅当 $F$ 保持所有小余极限并且是可达函子。函子 $G:\mathcal D\to\mathcal C$ 有左伴随，当且仅当 $G$ 保持所有小极限并且是可达函子。
