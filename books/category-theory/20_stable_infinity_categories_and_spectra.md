...
在普通导出范畴中，三角结构记录纤维与余纤维，却不保留产生这些三角的全部高阶信息。稳定 $\infty$-范畴要求零对象存在，并且推出方形恰好是拉回方形；于是纤维与余纤维通过悬挂互相决定，映射对象自然提升为谱。取同伦范畴会得到三角范畴，但稳定 $\infty$-范畴还保留函子、极限和同伦之间的相干数据。本章从稳定性等价条件出发，构造纤维序列、映射谱和谱范畴的普适性质。

所需背景是 $\infty$-范畴中的有限极限、有限余极限、零对象和映射空间。三角公理作为稳定结构的后果推导；不会反过来假定任意三角范畴都有唯一稳定增强。
...
**外部输入定理 20.L（有限滤过对象的谱序列）.** 设 $C$ 带有 t-结构，并设 $X$ 有有限递增滤过

$$
0=F_{-1}X\to F_0X\to\cdots\to F_NX=X.
$$

则 exact couple 构造给出一个有限因而强收敛的上同调谱序列
...
其 $d_r$ 的双次数为 $(r,1-r)$。该谱序列的极限带有由 $F_\bullet X$ 在 $H^*(X)$ 上诱导的有限滤过，且 $E_\infty$ 是该滤过的 associated graded。对无界或非完备滤过，必须另行验证穷尽性、完备性与导出极限条件；本定理不作此类断言。
