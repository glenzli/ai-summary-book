...
普通范畴的 nerve 不只记录对象和态射，也把每串可复合态射编码成高维单纯形；内角填充在其中表达复合的存在与相干唯一性。Quasi-category 保留这一内角条件，却允许填充空间非平凡，从而把高阶同伦纳入“范畴”本身。本章从单纯形范畴 $\Delta$ 和单纯集的面、退化算子开始，构造标准单纯形与角，最后证明普通范畴 nerve 是 quasi-category，并说明外角与可逆性之间的关系。

这里只要求熟悉范畴、函子和反范畴；单纯复形的几何直觉有帮助但不是前提。后续的映射空间、极限、伴随与 Cartesian fibration 都依赖本章的 inner horn 口径，因此每个 horn inclusion 和填充条件都会写明维数与缺失面。
