...
预层允许在每个测试对象上记录数据，却没有强迫局部数据能够唯一粘合。Grothendieck 拓扑用覆盖筛编码“哪些局部族足够”，sheaf 条件则把匹配族与全局截面之间的关系写成极限。Sheaf 化是从预层范畴到 sheaf 范畴的左正合反射，而 Grothendieck topos 抽象出由某个站点得到的全部这类范畴。本章从覆盖筛与 representable presheaf 开始，证明 sheaf 条件的几种形式，并说明 Giraud 公理怎样反向识别 topos。

本章使用预层、可表函子、极限、余极限和局部化思想。Giraud 定理及一般 sheaf 化存在性作为外部输入的部分会明确标出；站点大小、subcanonical 条件与“同一 topos 的不同呈示”也会分别说明。
...
**外部输入定理 14.14（Giraud 定理）.** 设 $\mathcal E$ 为局部小范畴。则 $\mathcal E$ 等价于某个小站点上的 sheaf 范畴，当且仅当：

1. $\mathcal E$ 有有限极限与所有小余极限；
2. 小余积是互不相交的，并且在拉回下稳定；
3. 每个内部等价关系都是某个态射的核偶，且其商在拉回下稳定；
4. $\mathcal E$ 有一个小生成族。

常见的等价公理表把第 2、3 条合写为“小余极限具有普适性且等价关系有效”。
