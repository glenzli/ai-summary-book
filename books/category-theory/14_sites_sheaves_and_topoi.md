...
**定义 14.A.** 预层 $F$ 称为 separated，若对每个覆盖筛 $S$，映射
...
**外部输入命题 14.C.** plus 构造把预层送到 separated 预层；若 $F$ 已 separated，则 $F^+$ 是 sheaf。在许多标准站点口径下，sheaf 化可由
...
**外部输入定理 14.6（sheaf 化）.** 对任意小站点 $(\mathcal C,J)$，包含函子
...
**定义 14.7.** Grothendieck topos 是与某个小站点的 sheaf 范畴
...
**命题 14.8.** 每个预层范畴 $\widehat{\mathcal C}$ 是 Grothendieck topos。
...
**定义 14.D.** Grothendieck topoi 之间的几何态射
...
的 inverse image 部分 $a$，因为定理 14.6 说明 $a$ 左正合。

**外部输入定理 14.9（Giraud 定理）.** 一个范畴是 Grothendieck topos，当且仅当它满足一组内在公理：有小余极限、有限极限、余极限与拉回有适当相容性、等价关系有效，并有小生成族等。

本书后续只使用该定理的方向性解释；完整陈述和证明见 Johnstone、Mac Lane-Moerdijk 和 SGA 4。

## 14.6 本章小结

站点把覆盖关系从拓扑空间推广到一般范畴。sheaf 是满足覆盖粘合条件的预层。Grothendieck topos 是 sheaf 范畴，是集合范畴的广义版本，也是几何、逻辑和高阶 topos 的入口。
...
**练习 14.4.** 证明 sheaf 范畴中的有限极限由预层有限极限 sheaf 化得到，并指出定理 14.6 的作用。

**练习 14.5.** 查阅 Giraud 定理的完整公理列表，并与定义 14.7 比较。
