...
自然变换要求一族分量同时满足所有态射的相容条件，end 把这种“对所有对象取积再施加相等关系”的结构压缩成一个极限；coend 则对偶地把两侧作用沿态射关系取商。由此，自然变换对象、张量积、迹、Day 卷积和 co-Yoneda 公式都能用统一的积分式记号表达。本章不会把 $\int_c$ 当作形式符号，而会从楔与余楔的泛性质出发，推导等化子、余等化子和 Fubini 公式所需的完备性条件。

读者需要函子范畴、极限、余极限、Yoneda 与幺半范畴。所有索引范畴和（余）积大小都沿用前文 universe 约定；交换两个 end 或 coend 时，会明确指出相应极限/余极限为何存在并可交换。
