...
在 quasi-category 中，复合不再是指定的二元运算，极限也不能只要求 Hom 集上的双射；正确的泛性质必须比较映射空间并得到同伦等价。类似地，一条边是否为等价，要在同伦范畴中可逆，伴随则可由单位、余单位的相干高阶数据或映射空间等价刻画。本章把普通范畴论的等价、终对象、极限和伴随逐项提升到这一同伦不变口径，并检查取同伦范畴后能恢复哪些经典结论、又会遗失哪些信息。

本章只依赖第十七章的单纯集、inner horn 与 quasi-category。涉及 mapping spaces、join 和 overcategory 的构造会在使用前定义；外部输入的高阶伴随判据也会明确假设，而不把 1-范畴的三角恒等式直接搬入高阶环境。

## 18.1 同伦范畴

**定义 18.1.** 设 $C$ 为 quasi-category。其同伦范畴 $hC$ 的对象是 $C_0$ 的元素。态射 $x\to y$ 是从 $x$ 到 $y$ 的 1-单纯形按 2-单纯形生成的同伦关系取商。

**外部输入定理 18.2.** 上述关系给出范畴 $hC$，复合由 $\Lambda_1^2$ 的填充定义，并且不同填充给出同一同伦类。
