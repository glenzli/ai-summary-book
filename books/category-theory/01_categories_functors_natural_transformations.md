...
范畴论不从对象的内部元素出发，而从对象之间可以怎样复合态射出发。函子保存这种复合结构，自然变换则比较两个函子而不依赖逐对象的任意选择；完全忠实、本质满和范畴等价由此成为比“对象集合相等”更合适的结构比较。后续的泛性质、极限、伴随和高阶范畴都会反复使用这三层对象，因此本章从定义开始，同时用群、偏序集和代数结构建立可计算的例子，不假定读者已有范畴论背景。

大小问题从一开始便纳入记号。本文采用附录 A 的默认轮廓：对象与总态射在 $\mathcal V$ 层小，每个 Hom 在 $\mathcal U$ 层小；未经额外说明，“小范畴”特指 $\mathcal U$-小范畴。这个区分将在函子范畴和 Yoneda 嵌入中变得不可省略。
