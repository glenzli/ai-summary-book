...
给定两类态射 $\mathcal E$ 与 $\mathcal M$，正交性要求每个 $e\in\mathcal E$ 对每个 $m\in\mathcal M$ 的提升存在且唯一；若每个态射还能按 $m\circ e$ 分解，便得到因子化系统。弱因子化系统只要求提升存在，因而允许同伦意义下的选择，并成为模型范畴中 cofibration--trivial fibration 配对的基础。反射局部化、像分解和小对象论证都可以用这一语言比较。

本章只使用交换方形、提升性质、极限/余极限与模型范畴定义。Functorial factorization、algebraic weak factorization 与 small object argument 需要额外小性条件，正文会在调用时单独陈述。
