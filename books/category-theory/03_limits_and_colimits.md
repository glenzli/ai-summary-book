...
积、拉回、逆极限和解空间看似来自不同领域，实质上都在寻找某个图形的泛锥；余积、推出和商对象则是对偶的泛余锥。把它们统一为极限与余极限后，不但可以一次证明唯一性和函子性，还能把“所有有限极限”的存在性压缩为终对象、二元积和等化子的存在。本章以第二章的可表性观点定义锥，随后推导逐点极限、有限生成准则和对偶结论。

索引范畴 $\mathcal J$ 默认 $\mathcal U$-小，目标范畴 $\mathcal C$ 默认局部 $\mathcal U$-小，所以锥态射与所有 Hom 集仍属于 $\mathbf{Set}_{\mathcal U}$。若把图形扩大到 $\mathcal V$ 层，相应的完备性假设也必须同步加强。
