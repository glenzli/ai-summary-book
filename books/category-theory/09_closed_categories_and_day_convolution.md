...
在集合中，二元函数可以柯里化为到函数集的映射；闭幺半范畴把这一现象表达为 $-\otimes X\dashv [X,-]$。内部 Hom 让“态射对象”留在原范畴中，并为富范畴提供基础。另一方面，若一个小幺半范畴嵌入其预层范畴，希望张量能沿 Yoneda 延拓并保持余极限，Day 卷积正给出满足这一要求的结构。本章先研究闭结构的求值与余求值，再由 Kan 延拓和 coend 公式构造 Day 卷积。

所需背景是幺半范畴、伴随和预层。Coend 的完整定义推迟到第十一章；本章出现该公式时会明确把它作为可由余等化子展开的记号，而不依赖尚未建立的交换定理。
