# 附录 G：HMS theorem locator 与外部输入索引

本附录给出 theorem locator 的教材本体索引。在线教材最低闭合版详见 [ONLINE_THEOREM_LOCATOR.md](ONLINE_THEOREM_LOCATOR.md)；本附录保留按主题组织的外部输入索引。当前不声称出版社级页码、出版版本差异和逐条定理编号校勘。
...
1. 每个外部输入都有一手资料入口；
2. 每个外部输入在 [EXTERNAL_INPUT_USAGE_TABLE.md](EXTERNAL_INPUT_USAGE_TABLE.md) 中标出使用章节、假设、输出和不内部证明的原因；
3. 研究边界与基础定理链已经分开；
4. 出版级页码、精确定理编号和版本差异校勘仍未执行，但这不影响在线教材本体闭合。
