...
本账本记录当前书稿中的内部命题、外部输入定理和研究边界。状态会随后续章节扩写更新。

## 内部命题

- **0.5**：dg/$A_\infty$ quasi-equivalence 诱导 $H^0$ 范畴等价。依赖：quasi-equivalence 定义。
- **1.4**：$H^0(\mathcal A)$ 是普通范畴。依赖：dg 复合为链映射、严格结合律、闭单位。
...
- **K.2**：生成元比较模板的形式证明。依赖：perfect modules 厚闭包。
- **K.6**：Cech diagrams 逐点 Morita 等价推出 homotopy colimits 等价。依赖：Morita homotopy category。
- **K.8**：stop removal quotient 中 linking disk 生成闭包对象被送零。依赖：quotient functor。
...
- **9.12**：Polishchuk-Zaslow 椭圆曲线 HMS。
- **10.5**：toric disk potential。来源：Cho-Oh/FOOO。
- **10.8**：Jacobian ring 与 quantum/Floer closed-string 结构对应。
