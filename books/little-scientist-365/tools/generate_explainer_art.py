...
# Twelve diagrams per month. Topics are selected when a picture can explain a
...
def render_network(diagram: Diagram, palette: tuple[str, str, str, str]) -> str:
    _, primary, accent, dark = palette
    positions = [(600, 340), (250, 190), (950, 190), (600, 525)]
    parts: list[str] = []
    for i, (node, (x, y)) in enumerate(zip(diagram.nodes, positions)):
        if i:
            parts.append(line(600, 340, x, y, primary if i % 2 else accent, 7, False))
        parts.append(node_card(node, x, y, 245 if i else 285, 210 if i else 245, palette, 0.5 if i else 0.62))
    return "".join(parts)
...
def validate_selection() -> None:
    if len(DIAGRAMS) != 144:
        raise RuntimeError(f"expected 144 selected diagrams, found {len(DIAGRAMS)}")
    ...
    if counts != Counter({index: 12 for index in range(12)}):
        raise RuntimeError(f"selection is not twelve diagrams per month: {counts}")
...
def main() -> None:
    ...
    if FALLBACK_KEYS:
        print("abstract_fallbacks=" + ",".join(sorted(FALLBACK_KEYS)))
