...
**定义 3.1（梯度显著性）.** 若 $x\in U$ 且 $F$ 在 $x$ Fréchet 可微，局部梯度归因为
...
**命题 3.2（局部零梯度不蕴含有限无贡献）.** 存在光滑函数 $F$ 与点 $x,x'$，使 $\nabla F(x)=0$，但 $F(x)-F(x')\ne0$。
...
**定义 3.3（可容许路径）.** 给定输入 $x\in U$ 与基线 $x'\in U$，可容许路径是绝对连续映射 $\gamma:[0,1]\to U$，满足 $\gamma(0)=x'$、$\gamma(1)=x$，并满足：
...
**定理 3.5（路径 completeness）.** 对定义 3.3 的任意可容许路径，
...
**定义 3.6（直线积分梯度）.** 若线段 $[x',x]\subset U$，取直线路径

$$
\gamma(\alpha)=x'+\alpha(x-x'),\qquad\alpha\in[0,1],
$$

并假设它满足定义 3.3。此时 $\dot\gamma_i=x_i-x_i'$，定义
...
**推论 3.7（integrated gradients 的 completeness）.** 在定义 3.6 的假设下，

$$
\sum_{i=1}^d\operatorname{IG}_i(x;x')=F(x)-F(x').
$$

**证明.** 将直线路径代入定理 3.5 即得。证毕。

completeness 是相对于端点和路径的守恒恒等式，不证明每个坐标归因具有唯一因果意义。换基线会改变总差，换路径可在总差不变时重新分配各项。[S01](SOURCES.md#s01) 提出原始 IG 方法及其公理；[S02](SOURCES.md#s02) 讨论其函数空间与唯一性主张的额外条件。本书只证明上述 completeness，不声称由它推出唯一性。
...
**练习 3.1.** 对 $F(x_1,x_2)=x_1x_2$、基线 $(0,0)$ 计算直线积分梯度。

**练习 3.2.** 对同一函数改用折线路径：先改变 $x_1$，再改变 $x_2$。比较路径归因。

**练习 3.3.** 给出梯度很大但有限合法扰动不改变离散输出的例子。

**练习 3.4.** 证明线性函数 $F(x)=w^\top x+b$ 的积分梯度为 $w_i(x_i-x_i')$。

**练习 3.5.** 为文本 embedding 选择两个不同基线，说明各自的语义困难。
