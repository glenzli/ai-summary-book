...
若研究只保存了热图，它完整回答了第一问，对后三问至多提供候选位置。一个可执行的后续方案是：先在独立实体集合上确认路由模式，再分别 patch 键、值和头输出，最后检验所得组件集合是否预测未见模板中的干预方向。这里的对象、操作、观察和推断边界都已分开；第十一章会把这条路线扩展成完整协议。
...
**练习 0.1.** 为“为什么模型回答巴黎”分别写出执行层、行为层和叙事层问题。

**练习 0.2.** 选择一种常见可视化，写出它能直接测量的量和两个不能直接推出的结论。

**练习 0.3.** 用六元组 $E=(T,\Xi,Q,M,C,R)$ 描述一次最小解释实验，并区分目标系统、被解释项与解释问题。

**练习 0.4.** 解释为什么局部充分性不蕴含组件在自然执行中被实际使用。
