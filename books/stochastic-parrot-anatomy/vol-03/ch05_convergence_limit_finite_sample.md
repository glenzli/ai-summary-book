...
**外部输入 P5.7（依概率收敛蕴含依分布收敛）.** 若 $X_n\xrightarrow{\mathbb P}X$，则 $X_n\Rightarrow X$。这是 Portmanteau 理论的标准推论。反向一般不成立；若 $X_n$ 与 $X\equiv c$ 定义在同一概率空间上，则 $X_n\Rightarrow c$ 等价于 $X_n\xrightarrow{\mathbb P}c$。所用版本与来源见 [SOURCES.md](SOURCES.md) 的弱收敛条目。
