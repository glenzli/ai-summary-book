...
使用 SGD（关于 SGD 的详细数学推导请参阅 [附录 A.1](../appendices/learning-notes/a.1_optimization_basics.md)），每次随机选取一个误分类点 $(\mathbf{x}_i, y_i)$ 进行更新。
...
如果数据是**线性可分 (Linearly Separable)** 的，PLA 保证在有限步内收敛找到一个解。（详细数学证明请见 **[附录 A.2](../appendices/learning-notes/a.2_perceptron_convergence.md)**）。
...
在此先用一维直观说明；正式定理条件、Cybenko 证明概要与 ReLU 分段线性构造见 **[附录 A.5](../appendices/learning-notes/a.5_universal_approximation.md)**。连续函数可由足够细的简单基函数或分段线性插值近似，但多维结论仍需正式密度定理。
...
*(关于 Logits 如何转化为概率的详细数学机制，请参阅 **[附录 A.7 Softmax 与 Cross-Entropy](../appendices/learning-notes/a.7_softmax_crossentropy.md)**)*
...
> **本章收束**：
> 本章从符号主义与连接主义进入感知机、MLP、SVM 与树集成，建立了“表示从哪里来、容量受什么限制、目标如何训练”的基本问题。下一章转向深度网络的训练机制、CNN 与序列模型，说明这些问题如何在可微多层模型中展开。
