...
Transformer 在 NLP 中取得广泛应用后，研究者也系统评估了它处理图像的能力。ViT 的关键做法之一是把图像 patch 表示为序列，并在大规模预训练后迁移到图像分类任务（[Dosovitskiy et al., 2020](SOURCE_NOTES.md#ref-dosovitskiy-2020)）。
...
Radford 等人提出的 **CLIP** 是多模态对比预训练的代表工作。它不训练固定的 ImageNet 1000 类输出头，而是在 batch 内把图文匹配写成双向对比分类目标（[Radford et al., 2021](SOURCE_NOTES.md#ref-radford-clip-2021)）。
...
直觉上，它优化“正确配对”在每行/每列候选中的相对对数概率；有限训练并不保证每个正例最终都严格排名第一。（更完整的 InfoNCE/对比学习推导见 **[附录 A.12](../appendices/learning-notes/a.12_contrastive_learning.md)**）

### 4. LLaVA: 大语言模型看世界 (Large Language-and-Vision Assistant)

CLIP 只能做匹配，不能生成自然语言回答。如果希望语言模型基于图像进行问答，需要把视觉特征映射到语言模型可以处理的表示空间。
**LLaVA** 给出了一种结构简单且影响很大的方案：用投影层连接预训练视觉编码器与自回归语言模型，再以视觉指令数据联合适配（[Liu et al., 2023](SOURCE_NOTES.md#ref-liu-llava-2023)）。
...
公开资料展示了几种不同层级的统一。GPT-4o 系统卡讨论端到端文本、视觉和音频训练及实时音频风险，可作为弱化“ASR -> 文本模型 -> TTS”串联管线的公开证据，但不能自动外推到其他产品（[OpenAI, 2024](SOURCE_NOTES.md#ref-openai-gpt4o-2024)）。Gemini 1.5 技术报告覆盖长文档、视频与音频（[Reid et al., 2024](SOURCE_NOTES.md#ref-reid-2024)）；截至 2026 年 7 月 12 日，Gemini 3.5 Flash 官方页列出 text/image/video/audio/PDF 输入、text 输出和 1,048,576 token 输入上限，并明确不支持 Live API（[Google, 2026](SOURCE_NOTES.md#ref-google-gemini-35-flash-2026)）。这说明它可作长上下文理解与工具型 Agent 的接口例子，却不是实时音频输出模型。

视频生成又有不同实现。Sora 2024 技术报告明确描述时空 patch 上的扩散 Transformer（[OpenAI, 2024](SOURCE_NOTES.md#ref-openai-sora-2024)），VideoPoet 则公开了自回归多模态语言模型路线（[Kondratyuk et al., 2023](SOURCE_NOTES.md#ref-kondratyuk-2023)）。Veo 官方材料记录能力与评测，但公开架构细节不能与 Sora 等量齐观。多模态接口相似，不代表内部生成目标相同；关于这些模型与世界模型的关系，见 **[6.6 世界模型与生成式媒体](ch06_multimodal_agents_world_models.md#section-6-6)**。
...
下一步的关键不只是增加模态，还包括跨模态对齐、时间推理、证据校准和行动反馈。相关延伸见 **[6.6 世界模型与生成式媒体](ch06_multimodal_agents_world_models.md#section-6-6)**。
<a id="section-6-2"></a>
...
Wei 等人的 **思维链 (Chain of Thought, CoT)** 工作研究了包含推理步骤的 few-shot 示例（[Wei et al., 2022](SOURCE_NOTES.md#ref-wei-cot-2022)）；单独加入 *“Let's think step by step”* 的 zero-shot 方法来自 Kojima 等人的后续工作（[Kojima et al., 2022](SOURCE_NOTES.md#ref-kojima-2022)）。两者在部分多步任务上可提高准确率，但收益依赖模型规模、任务和提示。
...
公开资料中，OpenAI 的 o1 发布材料报告了大规模 RL 与测试时计算的关系（[OpenAI, 2024](SOURCE_NOTES.md#ref-openai-o1-2024)），DeepSeekMath 与 DeepSeek-R1 论文则公开 GRPO 和可验证奖励（[Shao et al., 2024](SOURCE_NOTES.md#ref-shao-deepseekmath-2024)；[DeepSeek-AI, 2025](SOURCE_NOTES.md#ref-deepseek-r1-2025)）。这些工作先在训练阶段用监督数据、强化学习或验证器提高成功轨迹的概率，再在推理阶段为困难问题分配更多 token、候选采样、搜索或外部校验。接口通常只返回最终答案、摘要或面向用户的可审计说明；不能从是否显示“思考”反推内部完整轨迹，也不能把其他产品的相似界面视为相同训练配方。

这类模型在数学、代码、科学问答上提升明显，但它们不是万能证明器：事实性、长程规划、现实工具执行和安全边界仍然需要外部验证。

### 3. ReAct: 推理与行动 (Reasoning + Acting)

工具使用不要求模型先拥有一份完美计划。**推理-行动 (ReAct, Reasoning + Acting)** 框架把任务展开成推理文本与环境动作交替的轨迹，让新观察能修正下一步选择（[Yao et al., 2022](SOURCE_NOTES.md#ref-yao-react-2022)）。
...
这套最小闭环解释了语言模型怎样从回答者变成控制器，却还没有解决协议发现、持久状态、跨 Agent 协作和安全审计。关于 2026 年前后更完整的 Agent 运行时、MCP/A2A 协议、上下文工程、多 Agent 编排、权限安全与轨迹评测，见 **[6.5 Agent 系统工程](ch06_multimodal_agents_world_models.md#section-6-5)**。
<a id="section-6-3"></a>
...
RAG 是一种将**检索系统**与**生成模型**结合的混合架构。它可以在生成时引入训练参数之外的语料；这些语料是否实时、私有或可信，取决于索引更新、访问控制和数据治理。原始 RAG 工作把检索到的文档作为潜变量接到知识密集型生成任务中（[Lewis et al., 2020](SOURCE_NOTES.md#ref-lewis-rag-2020)）。
...
**MCP (Model Context Protocol)** 可以理解为“LLM 应用连接外部上下文与工具的通用接口”。截至校准日，当前协议版本为 2025-11-25（[Model Context Protocol, 2025-11-25](SOURCE_NOTES.md#ref-mcp-2025-11-25)）。server 可以公开供应用读取的 resources、由用户选择的 prompts，以及可产生查询或副作用的 tools；client 则在初始化时声明 sampling、roots、elicitation 等能力。对象类型区分了谁控制上下文、谁发起动作，也为能力协商和权限判断提供了结构。
...
如果 MCP 主要解决单个 Agent/LLM 应用如何接入工具和上下文，那么 **A2A (Agent2Agent)** 主要解决 Agent 之间如何通信与协调。A2A 由 Google 发起、后进入 Linux Foundation 项目；官方 v1.0.0 是 2026 年发布的首个稳定版，本文按该版本描述（[A2A Project, 2026](SOURCE_NOTES.md#ref-a2a-2026)）。
...
“世界模型”没有单一跨领域定义。广义上，它学习环境状态及其时间演化；在 model-based RL/控制语境中，动力学通常还以动作作为条件，并支持奖励预测或规划。World Models 论文用压缩的时空表示训练控制策略，是这一路线的经典实例（[Ha & Schmidhuber, 2018](SOURCE_NOTES.md#ref-ha-schmidhuber-2018)）。被动视频预测可学习环境统计，但不必然具备可行动规划接口。
...
早期世界模型路线常见于 model-based RL。World Models、PlaNet、Dreamer 等方法让智能体在潜在空间中学习环境动态，并用想象轨迹训练策略；Dreamer 明确在学到的潜在动力学中传播价值梯度（[Hafner et al., 2019](SOURCE_NOTES.md#ref-hafner-2019)）。它们的核心不是生成高分辨率视频，而是用紧凑状态预测未来，从而减少真实环境交互成本。

JEPA (Joint Embedding Predictive Architecture) 强调在表示空间预测，而不是逐像素重建。这里引用的 I-JEPA 从同一张静态图像的上下文块预测目标块表示（[Assran et al., 2023](SOURCE_NOTES.md#ref-assran-ijepa-2023)）；它没有动作条件或时间动力学，不能直接归类为完整世界模型，但展示了“预测语义表示而非像素”的相关设计原则。

Genie 把问题推进到“从视频中学习可交互世界”：时空 tokenizer 压缩视频，自回归动力学模型预测后续表示，潜在动作模型则从没有动作标签的视频中提取可控变化（[Bruce et al., 2024](SOURCE_NOTES.md#ref-bruce-genie-2024)）。用户对生成环境施加潜在动作时，模型逐帧延续结果。这说明世界模型不一定先有显式物理引擎，也可以从视频中学习统计动力学；可控生成仍不等同于已经获得真实世界的因果模型。
...
**扩散模型 (Diffusion Models)** 通过逐步加噪/去噪学习数据分布，在图像质量和训练稳定性上取得重要突破。Latent Diffusion 把扩散过程放到压缩潜在空间中进行，大幅降低计算成本，也推动了 Stable Diffusion 等开放生态（[Rombach et al., 2022](SOURCE_NOTES.md#ref-rombach-2022)）。
...
**Diffusion Transformer (DiT)** 在其潜在扩散实验中以 Transformer 替代常见 U-Net 主干，并展示模型规模、训练计算与生成质量指标的经验扩展关系（[Peebles & Xie, 2023](SOURCE_NOTES.md#ref-peebles-xie-2023)）。它不意味着所有生成 Transformer 都遵循与语言模型相同的缩放定律。

**Flow Matching** 直接回归把基分布输运到数据分布的连续速度场（[Lipman et al., 2022](SOURCE_NOTES.md#ref-lipman-2022)）；Rectified Flow 是相关但有特定路径构造与重流化思想的路线。它们与扩散模型存在数学联系，但训练目标和采样 ODE/SDE 口径不能无条件互换。实际系统还可能结合蒸馏和多阶段解码。
...
这些系统代表不同证据层级与架构路线：Sora 2024 技术报告公开了扩散 Transformer 和时空 patch（[OpenAI, 2024](SOURCE_NOTES.md#ref-openai-sora-2024)），VideoPoet 公开了自回归多模态语言模型路线（[Kondratyuk et al., 2023](SOURCE_NOTES.md#ref-kondratyuk-2023)）；Veo 官方页面披露能力和评测，却没有提供与 Sora 等量的架构细节。OpenAI 报告标题中的 “world simulators” 是研究假设与方向，不是已证明结论；视频模型仍会在因果、物理、空间关系和精确计数上失败。
...
本节补上了“世界模型”和“生成式媒体”的主线。回看 **[6.5 Agent 系统工程](ch06_multimodal_agents_world_models.md#section-6-5)** 时，读者应区分两点：许多工具型 Agent 只依靠语言模型、状态机和环境反馈即可运行，并未显式训练世界模型；需要长期规划或物理控制时，能够预测行动后果的显式或隐式动力学表示会更重要。视频生成、语言预测表示与控制世界模型之间有研究联系，但不能互相等同。

技术演进到这里已经从单一文本模型扩展到多模态制品、工具闭环和环境预测。下面把这些路线压到分布生成、跨模态表示、状态转移与制品 provenance 四类对象上；这不是第二遍产品综述，而是为第七章的工件与系统边界准备形式接口。

## 生成媒体、多模态与世界模型的审计接口

语言模型不是现代 AI 的全部。图像扩散模型生成像素制品，多模态模型把文本、图像、音频和视频接到同一接口，世界模型则试图在潜在状态中预测环境演化。它们扩大了模型的输入输出类型，也扩大了证据负担：一段文字的来源问题会变成图像编辑史、音频说话人归属、视频时序一致性和交互环境中的行动后果。

本章不按产品列举模型，而以三种数学接口组织材料：分布生成、跨模态表示和状态转移。三者都能产生逼真制品，却支持不同的主张。

### S3.1 三类生成接口

设数据空间为可测空间 $\mathcal X$；在潜变量接口中另取 $z\in\mathbb R^m$。生成模型至少有三种常见接口。

**自回归接口**把对象编码为离散序列 $x_{1:n}$，并分解为

$$
p_\theta(x_{1:n})=\prod_{t=1}^n p_\theta(x_t\mid x_{<t}).
$$

文本 token、图像 token、音频 codec token 和视频 token 都可进入这个形式。分解依赖编码和次序；它不是对对象语义的唯一分解。
...
变分自编码器用近似后验 $q_\phi(z\mid x)$ 优化证据下界。潜空间适合压缩和编辑，但某个潜方向的可读性不自动说明生成机制唯一使用该语义。

**逐步去噪接口**先定义正向加噪过程，再学习反向恢复。离散时间扩散的典型形式为
...
其中 $0<\beta_t<1$。模型学习噪声、score 或相邻反向核的参数。最终样本取决于初始噪声、调度、条件输入、采样器和数值实现；只保存文本提示不足以复现图像。

### S3.2 对抗生成与分布判别

生成对抗网络以生成器 $G$ 和判别器 $D$ 的博弈为核心。经典目标写作
...
这个目标比较数据分布与生成分布，不为单个样本提供事实来源。即使判别器无法区分两个分布，也不能推出某张生成图像记录了现实事件。分布拟合、感知逼真和事实真实性必须分开。

### S3.3 跨模态表示

设文本编码器 $f_T$ 和图像编码器 $f_I$ 把样本映到共同向量空间。对比学习可使配对样本的相似度高于未配对样本。对一个批次，单向对比损失可写为

$$
\ell_i
=
-\log
\frac{\exp(\langle f_I(i),f_T(t_i)\rangle/\tau)}
{\sum_j\exp(\langle f_I(i),f_T(t_j)\rangle/\tau)},
$$

其中 $\tau>0$。该目标建立统计对齐，不建立严格语义同一。若图像与文本在训练数据中经常共现，模型可能获得可用检索表示；它仍可能依赖背景、排版或数据偏差。

多模态生成系统通常再加入投影器、交叉注意力、视觉 tokenizer、音频编码器或统一序列模型。审计时至少记录：模态编码版本、预处理、时间采样、分辨率、截断、对齐模板和安全过滤。

上述损失在固定批次上就是“正确配对索引”的交叉熵；其数值随负样本集合、相似度、温度和批内依赖改变。把它解释为一般互信息下界还需要额外采样和密度比条件，不能由这条有限批次公式直接推出。有限 InfoNCE 梯度和这些边界见附录 C。

### S3.4 从像素到制品

文本输出常可由 Unicode 和 token 层描述；图像、音频和视频还需要制品层：

| 对象 | 最小记录 |
| --- | --- |
| 图像 | 像素格式、尺寸、色彩空间、编码器、seed、采样器、编辑掩码 |
| 音频 | 采样率、声道、codec、时间范围、说话人条件、后处理 |
| 视频 | 帧率、分辨率、时间采样、关键帧、音轨、剪辑和插帧记录 |
| 混合文档 | 组件清单、布局、字体、嵌入资源、导出工具和 digest |

相同的可见图像可以来自不同编辑史；相同 prompt 可以产生不同制品；相同 latent seed 在模型、调度或浮点实现改变后也未必复现。身份判断必须说明比较的是原始字节、解码像素、感知距离还是语义标签。

### S3.5 条件生成不是来源证明

给定条件 $c$ 的生成分布 $p_\theta(x\mid c)$ 只描述模型接口。若条件写“2025 年某城市的真实洪水”，样本看起来像新闻照片，并不意味着它指称真实事件。来源证明需要另外记录：
...
生成 provenance 也不能单独证明内容无害或合法，但它能阻止“合成制品被误当作相机记录”这一类类型错误。

### S3.6 世界模型的状态接口

世界模型试图学习环境的状态转移和观测生成。最小形式可写为

$$
z_{t+1}\sim K_\theta(\cdot\mid z_t,a_t),
\qquad
o_t\sim O_\theta(\cdot\mid z_t),
$$

其中 $z_t$ 是潜在状态，$a_t$ 是行动，$o_t$ 是观测。若再有奖励或目标模型，Agent 可以在潜在轨迹上规划。

这里的“世界”是相对于训练分布、传感器、行动集合和损失函数构造的预测对象。视频预测逼真不等于物理规律正确；潜在状态支持控制不等于它与现实对象一一对应；短时滚动稳定不保证长期闭环稳定。

### S3.7 预测误差怎样累积

设确定性近似转移为 $\hat z_{t+1}=F_\theta(\hat z_t,a_t)$，真实抽象转移为 $z_{t+1}=F(z_t,a_t)$。若在相关域内 $F_\theta$ 对状态是 $L$-Lipschitz，且单步模型误差不超过 $\varepsilon$，则

$$
\|\hat z_{t+1}-z_{t+1}\|
\le
L\|\hat z_t-z_t\|+\varepsilon.
$$
...
L^t\|\hat z_0-z_0\|
+
\varepsilon\sum_{j=0}^{t-1}L^j.
$$

该界只在共享行动、范数、状态对应和 Lipschitz 假设成立时有效。它说明为什么单步预测优秀仍可能在长轨迹中漂移，也说明评测世界模型时必须报告时域和闭环条件。

### S3.8 多模态 Agent 的新增风险

当多模态模型接入工具时，感知误差会变成行动参数。图像中的错读文字可能触发错误转账；语音中的说话人误识别可能绕过授权；视频中的时间顺序误判可能造成错误告警。治理接口需要把感知输出标成带来源和不确定性的观察，而不是直接升级为控制指令。

一个稳健闭环至少区分：原始传感数据、预处理结果、模型观察、解释标签、策略选择、审批和最终行动。高风险动作应要求独立传感器、结构化校验或人工确认。

### S3.9 从生成能力到审计对象

自回归、潜变量、扩散、跨模态对齐和世界模型扩展了可生成对象，却没有消除系统边界。相反，它们引入更多编码器、采样器、制品格式、来源关系和闭环状态。本章由此收束到同一个判断：模型能力来自表示、训练、后训练与运行时系统的组合，任何产品或榜单条目都应回到机制、证据和时效边界中阅读。下一章将把这些模型工件放入完整部署系统，说明模型能力何时才成为一次可归因的系统行为。

### 练习

**练习 S3.1.** 为一张经局部重绘的生成图像设计最小 provenance 记录，区分原素材、掩码、文本条件、模型工件和人工后处理。

**练习 S3.2.** 说明为什么“同 prompt、同 seed”不足以复现扩散模型输出，并列出至少四个仍需固定的分量。

**练习 S3.3.** 在第 3.7 节假设下分别计算 $L<1$、$L=1$ 和 $L>1$ 时的误差上界形态，并解释其评测含义。

**练习 S3.4.** 给出一个多模态感知错误经 Agent 工具调用放大的例子，并设计一个阻断点。
