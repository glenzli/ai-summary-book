...
$$
\widehat R_n(\theta)=\frac1n\sum_{i=1}^n\ell_i(\theta),
$$

若小批量按声明的均匀方案抽样，并用与抽样方案匹配的平均或权重构造 $g_t$，则在当前参数条件下

$$
\mathbb E[g_t\mid\theta_{t-1}]
=\nabla\widehat R_n(\theta_{t-1}).
$$

因此小批量梯度是**经验风险完整梯度**的带噪无偏估计。若进一步把它解释为总体风险梯度估计，还需要训练样本来自目标总体、损失可积并允许交换梯度与期望等条件。SGD 更新为
...
将递推图沿时间展开后，仍可使用反向传播。定义

$$
J_j:=\frac{\partial h_j}{\partial h_{j-1}}.
$$

矩阵乘法不可交换，故较早状态对较晚状态的 Jacobian 按复合顺序明确写为
