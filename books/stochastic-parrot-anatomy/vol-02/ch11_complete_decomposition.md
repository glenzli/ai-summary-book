...
本章以贯穿案例的完整记录 $\mathcal O_\star$ 收束本卷。它不会把所有层压回一个字段；相反，十二个分量分别保存输入、配置、轨迹、表示、制品、语义与归属。记录的“完整”始终相对于事件 schema，而不是对现实中每个物理事件的全知描述。
...
“完整”指这些字段足以满足本卷规定的审计接口，不表示记录了现实中的所有物理微状态。
...
前十章已经逐一建立表示、运行、工具、来源、真值与责任对象。本节是参考索引，不是第二轮正文：`S5` 按输出记录的字段责任组织，`S6` 按本卷各章的语义接口组织。读者顺序阅读时可以直接跳到本卷结语；需要组装审计包时再由 locator 反查。字段若缺失，应记录带原因的缺失状态，而不是由模型文本补写。

### S5.1 表示链

记录原始字节、Unicode 标量序列、tokenizer 版本与 token 序列，并保存每条合法转换。内容相同不自动推出 token、渲染或来源相同。

### S5.2 运行轨迹

记录状态、事件、转移、错误、重试和终止原因。最终文本只是轨迹的一个投影，不能恢复未显示的工具调用或候选分支。

### S5.3 候选、提交与撤回

区分未提交候选、已显示前缀、持久写入和不可逆副作用。取消只约束取消点之后的合法提交；它不能使已发送或已写入事件从历史中消失。

### S5.4 Artifact 与 provenance

对每个制品记录内容身份、生成 activity、输入实体、工具版本和相关主体。摘要可支持选定规范化下的内容身份，不支持来源或规范归属身份。

### S5.5 真值与核验

把输出中的原子主张、语境、来源、核验状态和时间分开登记。来源报告、系统观察和现实事实属于不同终态。

### S5.6 规范元数据

作者、控制者、批准者、责任主体和受影响者分别记录。因果参与不蕴含作者身份或规范责任；归责还需要角色、控制、可预见性和制度规则。

### S5.7 输出记录

最小输出记录把前六项组合为类型化对象：表示链、运行轨迹、提交状态、制品与 provenance、主张表、规范元数据。完整不等于所有字段都有值，而是所有必要字段都有值或有类型化缺失原因。

### S6.1 字节、Unicode 与 token 的合法域

UTF-8 解码和 tokenizer 映射都是带域与版本的函数；宽容解码是另一个函数。记录不能把非法字节、替换字符和原输入合并成同一对象。

### S6.2 部分函数、关系与隐藏输入

失败、超时、拒绝和多结果系统通常需要部分函数或关系。把隐藏配置、环境和工具返回省略为“prompt 到输出的函数”会改变对象类型。

### S6.3 带标签转移系统

运行语义记录状态集合、事件标签和转移关系。轨迹是连续转移的有限或无限序列；观察者只看见由观察函数选择的投影。

### S6.4 自回归生成的小步语义

每次 token 选择依赖当前状态、模型工件、解码器和随机源。条件分布描述下一步候选，不把最终文本升级为事实概率或作者承诺。

### S6.5 工具与外部世界

工具调用把系统从文本生成扩展为外部交互。请求、返回、错误和副作用必须作为事件保存，工具返回既不是模型参数知识，也不是自动可信事实。

### S6.6 提交边界与幂等键

高风险调用要区分请求、接受、提交和回执。超时可能对应未知提交状态；幂等键支持查询和安全重试，但不能替代服务端语义与补偿方案。

### S6.7 流式输出与取消

显示流是提交前缀序列。前缀单调性只在已提交字符不被改写的协议下成立；候选 token、缓冲区和已显示内容必须分层。

### S6.8 并发与偏序

并发轨迹由 happens-before 偏序而非单一墙钟顺序定义。只有在可交换性、线性化点或其他同步条件成立时，才可把多个合法调度视为同一观察结果。

### S6.9 Provenance 与身份

内容身份、字节身份、制品身份和来源身份由不同关系定义。provenance 图说明实体、活动和主体的关联，不自动给出真值或责任结论。

### S6.10 真值、核验与语境

字符串相同不保证命题相同；命题相同也不保证当前为真。核验必须固定指称、时间、数据源和允许的证据规则。

### S6.11 规范归属

系统可以在因果上生成文本，却不因此成为法律或道德主体。规范归属由制度角色、授权、控制能力、审批与组织规则决定。

### S6.12 从运行对象到不确定性

完整分解得到的是可审计运行对象，不是确定性保证。概率、观察者不确定性、比较关系和复现判决还需要各自的数学对象。

有限偏序调度不变性、may/must/概率观察、有限随机核终止、完整幂等合同、确定归并和记录身份的强化证明见[附录 E](../appendices/app-e_operational_semantics_proof_kernel.md)。附录只承担这些强化结果，不重复本卷基础定义链。

## 11.12 本卷结语：一句话重新成为一条运行

沿着 $\mathcal O_\star$ 回看本卷，所谓“一次输出”已经不再是一个含混名词。它可以是 $v^{(b)}$、$u_\star$、$E_8(u_\star)$、$t_\star$、$e_u$、$p_\star$、两个事实主张或一组规范分类；每个对象都有自己的类型和同一关系。$\operatorname{WF}$ 所做的不是选出其中一个“真正输出”，而是检查它们之间的转换、证据和归属是否彼此一致。改变审计用途就会改变投影；保留这个选择，才是本卷最终得到的输出存在论。

这个结论仍没有消除随机性，也没有给“复现成功”规定唯一含义。它只提供了谈论这些问题之前不可省略的运行对象。下一卷将分别建立概率空间、观察函数、统计判决和可执行复现合同，使“可能发生什么”与“再次得到什么”不再混成一句话。
