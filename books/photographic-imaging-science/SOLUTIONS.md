...
答案编号与 18 个编号章节的练习一一对应。计算默认采用正文符号、线性响应和题目给定
的简化条件；实际器材测量还需按相应章节加入光谱、温度、读出模式和处理链。

## 序章

**答案 0.1.** “颜色更浓”至少可拆为：

1. 镜头在指定照明、光圈和像高下的相对光谱透射使某些波段比例不同；
2. 同一 RAW 解码矩阵和白平衡下，相机 RGB 的饱和度或色差不同；
3. 逆光时 veiling glare 较低，使局部颜色对比度较高；
4. 相机 JPEG 配置使用了更强的颜色矩阵或饱和度曲线。

前三项分别可用光谱仪、统一 RAW 工作流和固定逆光场景测量；第四项属于处理而非镜片
材料的直接性质。

**答案 0.2.** $100\%$ 显示固定“一传感器输出像素对应一个屏幕像素”，没有固定
整张照片的显示尺寸，也没有固定单位画面面积包含的采样数。61 MP 图会被显示得更大，
单像素接收面积也常不同。公平的固定输出方案是用同一 RAW 流程把两张图缩放到例如
6000 像素长边，在相同屏幕尺寸与观看距离下比较；同时固定视角、光圈/景深、快门和
场景曝光。

**答案 0.3.** 14 bit 只给最多 $2^{14}$ 个数字码。还缺少黑电平、白电平、量化
与读出噪声、满阱容量、ADC 前增益、线性范围和动态范围下端 SNR 判据。即使所有码
都可用，14 bit 也只是容器的理论码值比，不保证 14 档可用场景曝光。
