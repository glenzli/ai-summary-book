...
3.  **缺乏行动力**: 它们只能输出文本，不能直接操作外部环境或调用计算资源。
...
在让模型行动之前，首先要让它学会“思考”。
Wei 等人提出的 **思维链 (Chain of Thought, CoT)** 是一项重要发现：只要在 Prompt 中加入一句 *"Let's think step by step"*，或者提供包含推理步骤的示例，模型的逻辑能力就会大幅提升。
...
o1/o3、DeepSeek-R1、GPT-5.x thinking/reasoning 类模型、DeepSeek V4 Pro 等推理模型的共同趋势，是把“慢思考”从提示技巧推进为训练和推理机制：
...
1.  **大脑 (Brain)**: LLM 或 reasoning model，负责规划与决策。
2.  **记忆 (Memory)**:
    *   短期记忆：上下文窗口。
