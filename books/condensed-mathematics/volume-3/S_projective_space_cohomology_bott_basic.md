...
**证明.** 定理 S.2 已给出 $\mathcal O(d)$ 的非零上同调只可能位于
$q=0$ 或 $q=n$；因此所有严格中间次数均消失。证毕。
...
**证明.** 在定理 S.2 中取 $n=1$。当 $d\ge0$ 时，$H^0$ 的维数是一次
齐次坐标环中次数 $d$ 的单项式数，即 $d+1$；当 $d<0$ 时为零。顶次
上同调公式给出 $d\le-2$ 时维数为 $-d-1$，而 $d\ge-1$ 时为零。合并
两段即得两个最大值公式。证毕。
