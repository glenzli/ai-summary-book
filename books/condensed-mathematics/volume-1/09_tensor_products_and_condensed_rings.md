...
本章在 $\mathbf{CondAb}$ 中构造张量积，并定义凝聚环（condensed ring）。这是进入凝聚模、solid modules 和解析结构之前必须建立的代数基础。

## 依赖前置知识

需要第四章的凝聚阿贝尔群、第七章的自由对象，以及一般阿贝尔群张量积的泛性质。
...
1. $\mathbf{CondAb}$ 中张量积通过逐点张量后 sheafification 定义。
2. $\underline{\mathbb Z}$ 是张量单位。
3. 凝聚环是 $\mathbf{CondAb}$ 中的交换环对象。
4. 拓扑环自然给出凝聚环。

下一章将定义凝聚环上的模，并证明自由模与极不连通测试空间仍给出投射对象。
