...
第十二章引用 Nöbeling 定理来说明 solid 对象具有大量乘积型投射生成元。本附录整理相关计算，并给出可直接检查的特殊情形。一般 Nöbeling 定理作为本书的外部输入定理使用；第二卷会继续使用它，而不是在第一卷中重写 Bergman 的完整长证明。
...
并对有限支撑的坐标条件作良序归纳，构造一组由开闭条件函数生成的基。关键点是控制无限坐标集带来的相容性，避免一般“自由群的滤过并仍自由”这一错误命题。

本书需要的后果只有两个：

1. $C(S,\mathbb Z)$ 存在某个自由基。
2. 因而 $\operatorname{Hom}(C(S,\mathbb Z),-)$ 把它转化为乘积。

后续所有 solid 计算只用这两个后果。
