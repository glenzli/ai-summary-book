...
sheaf 满射只保证局部提升：给定 $b\in B(S)$，通常要先找覆盖 $S_i\to S$，才能把
$b|_{S_i}$ 提升到 $A(S_i)$。若覆盖的联合满射
$\coprod_iS_i\to S$ 有连续截面，局部提升便可沿截面拉回为 $S$ 上的全局提升。
所以我们需要的不是一般 profinite 测试空间，而是一类对所有紧 Hausdorff 满射都具有
提升性质的对象。

Gleason 定理把这种范畴投射性等价地刻画为极不连通性：每个开集的闭包仍开。结合
第二章的商映射和第五章的测试站点比较，这一拓扑条件将推出凝聚阿贝尔群在极不连通
空间上取值是正合函子。Gleason 定理本身作为精确外部输入使用，其后从分裂覆盖到
正合取值的 sheaf 论推导则在正文完成。
...
Gleason 投射性使每个覆盖极不连通空间的有限联合满射分裂，而 Gleason cover 又保证
每个紧 Hausdorff 空间都能被这类对象覆盖。两项性质合在一起，一方面说明极不连通
空间足以检测凝聚对象，另一方面把 sheaf 满射的局部提升升级为逐点满射，故
$A\mapsto A(E)$ 对每个极不连通 $E$ 都正合。下一章把这一取值函子表示为 Hom，
从而把拓扑测试对象转化成真正的投射生成元。
