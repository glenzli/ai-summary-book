...
对无限 profinite 集合 $S$，普通自由对象 $\mathbb Z[\underline S]$ 只允许有限线性
组合，因而不能表示在有限商上相容的全部整值测度。逆极限
$S=\varprojlim_iS_i$ 提示另一个对象：先在每个有限集合上取自由阿贝尔群，再沿有限商
取逆极限。所得
...
接收的元素不再只有有限支撑 Dirac 组合，这正是 solid 完成化要捕捉的差异。

利用前章的凝聚阿贝尔群、自由对象、投射分解与派生 Hom，我们把 solid 对象定义为
从 $\mathbb Z[\underline S]$ 到 $\mathbb Z^\square[S]$ 的延拓唯一性，并用
Nöbeling 定理分析其乘积模型。深层的反射局部化和阿贝尔范畴结构仍按 Scholze 讲义
作为精确输入；从定义得到的 Hom 判别与有限测试例则在正文中逐项验证。
...
## 12.8 从 Dirac 组合到整值测度

$\mathbb Z^\square[S]$ 把有限商上的自由群组成相容逆极限；solid 条件则要求从点的
Dirac 组合出发的映射唯一延拓到这些整值测度。Nöbeling 定理把
$C(S,\mathbb Z)$ 识别为自由阿贝尔群，因而给出
$\mathbb Z^\square[S]\cong\prod_I\underline{\mathbb Z}$ 的非典范乘积模型。
反射函子与派生局部化仍是第二卷所登记的外部结构输入。下一章只接受这项输入，并考察
普通张量后再次固化如何在 solid 对象之间产生闭合的乘法。
