...
第一卷的投射生成元和派生张量解决了“怎样在凝聚对象中做同调代数”，却没有回答哪些
对象代表合适的完成化，也没有回答允许何种无限线性组合。对整值测度，solidification
给出反射；换到底环与测度理论后，analytic localization 承担同样角色；进入实分析时，
普通 Radon 测度的失败又迫使我们采用 $<p$-测度与 liquid 对象。这三层结构彼此关联，
但任何一层都不能仅凭“完备”二字替代另一层。

本卷沿这条差异链推进：先把 solid 条件提升到无界导出范畴，再构造幺半代数；继而从
Dirac-to-measure 映射得到 analytic ring 与 Bousfield localization；最后讨论 liquid
实现、离散 Huber pair 的 rational descent 以及 $f_!\dashv f^!$ 的几何接口。第一卷
已经证明的凝聚基础继续作为内部工具，Scholze 与 Clausen-Scholze 的深层结构定理则以
精确外部输入出现。
...
自由 solid 对象的 Hom 判别和 Nöbeling 乘积模型提供了起点，但后续真正需要的是反射
局部化、张量相容与下降。第一章先把 solid 条件改写成一族 cone 的右正交条件；这一
形式会在第四章原样推广为 analytic localization，并最终让第六、七章能够从局部解析
环走向 rational 覆盖和带边界的推前。
