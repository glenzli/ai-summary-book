...
**使用边界.** 该定理承担“场重定义不改物理”的量子层面结论，精确来源定位见附录 B 的 EFT-EQ 项。命题 2.7 只证明有限维算符商空间中的线性基变换，不替代等价定理，也不覆盖 anomalous Jacobian、非局域或不可逆变换；高阶 EFT 截断还须保留场重定义诱导的高阶项，见第四章。
...
匹配比较的是同一 scheme、轻参数和 IR 处方下的重整化低能展开。共同 soft 非解析部分由 EFT 重现并相消，剩余 hard 局域多项式决定 Wilson 系数；多个分离阈值则须顺序 matching 与 running，不能把物理质量和匹配尺度混为一谈。
