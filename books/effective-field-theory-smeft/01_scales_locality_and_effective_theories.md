...
设一个轻粒子以动量 $p$ 交换质量为 $M$ 的重态。完整振幅知道传播子的 pole 在哪里，低能实验却只看到它在 $p^2=0$ 附近的若干 Taylor 系数。把这些系数改写为局域导数算符，便得到 EFT 最基本的计算机制；但这个说法只有在运动学域不碰到遗漏奇点时才成立。尤其要区分控制解析半径的物理谱隙 $M_{\rm gap}$、归一化 Wilson 坐标的参考尺度 $\Lambda_{\rm ref}$，以及组织重整化计算的匹配尺度 $\mu_{\rm match}$。从一个重传播子的可求余项开始，局域性、有效自由度和截断误差会成为同一套尺度分离陈述，而不再只是“低能看不见重粒子”的直觉口号。
...
只有 $M_{\rm gap}$ 控制局域 Taylor 展开的运动学边界。$\Lambda_{\rm ref}$ 可连同 Wilson 坐标一起重标度，$\mu_{\rm match}$ 则由匹配与 RG 的尺度抵消控制。在单重尺度例子中，UV 模型可先确定 $M_{\rm gap}=M$，随后再作坐标选择 $\Lambda_{\rm ref}=M$；即使三者数值接近，也不能据此把 $\mu_{\rm match}$ 当作物理质量。
...
重传播子的例子把三个尺度角色分开了：$M_{\rm gap}$ 决定 Taylor 展开能走多远，$\Lambda_{\rm ref}$ 只选择 Wilson 坐标，$\mu_{\rm match}$ 则安排阈值计算。给定自由度、对称性与冗余商后，EFT 是在固定低能域上使用的局域渐近作用量；它的预测力来自有限阶余项或幂计数，而不依赖无穷级数必然收敛。接下来需要由 UV 理论真正算出这些局域项的系数，这正是匹配所承担的工作。
