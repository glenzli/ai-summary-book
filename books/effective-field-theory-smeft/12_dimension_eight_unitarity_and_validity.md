...
高能尾部常使维数六振幅随 $E^2$ 增长，从而提高实验灵敏度；同一个增长也会放大维数六平方、维数八插入和多次插入，使线性截断最先失效。判断这一转折不能只比较某个拟合参考尺度，而要按总逆尺度次数 $p$ 展开振幅、输入参数和相空间，并把每个允许的同阶项列出。特别是 $|A_2|^2$ 与 SM 对 $A_4$ 的干涉都处在 $p=4$，却来自不同的计算对象。完成这项记账后，partial-wave unitarity 给出振幅增长的自洽性警告，前向色散关系的 positivity 条件则在额外解析性与 UV 假设下限制某些维数八组合。三者共同界定高能数据能提供什么信息，以及截断误差必须如何陈述。
...
**解释 12.4.** 高维算符振幅常随能量增长，如 $A_6\sim C E^2/\Lambda_{\rm ref}^2$。当该增长使 partial wave 接近 unitarity 界时，截断 EFT 已不能单独可信。

**例 12.5（常数角分布估计）.** 若某 $2\to2$ 振幅近似为
$$
{\cal A}(s,\cos\theta)=C{s\over\Lambda_{\rm ref}^2},
$$
则
$$
a_0(s)={1\over32\pi}\int_{-1}^{1}d\cos\theta\,{\cal A}
={C s\over16\pi\Lambda_{\rm ref}^2}.
$$
保守条件 $|{\rm Re}\,a_0|\le1/2$ 给出
$$
s\le {8\pi\Lambda_{\rm ref}^2\over |C|}.
$$
...
## 12.4 解释一个 SMEFT 限制所需的数据

**规则 12.8（SMEFT 限制的最小物理数据）.** 一个 SMEFT 限制应给出：
...
高能增长同时提高 Wilson 灵敏度和遗漏阶的重要性。固定假设下的 $p=4$ 预测必须把维数六平方、SM 与一次维数八插入的干涉、SM 与多次低维插入振幅的干涉，以及同阶输入参数和测度展开一起处理；只加平方项应明确标为部分方案。Partial-wave unitarity 提醒振幅何时已超出微扰描述，positivity 则在额外的解析性与 UV 条件下约束低能系数。它们与 $Q/M_{\rm gap}$ 检查互补，不能彼此替代。
...
**练习 12.2.** 若某算符给出振幅 $A=C s/\Lambda_{\rm ref}^2$，估计何时可能接近 partial-wave unitarity 边界，并说明该条件为何不单独确定 $M_{\rm gap}$。
