...
**外部输入 3.9（SMEFT 维数六一圈 RGE，SMEFT-RGE6）.** 在 baryon-number conserving Warsaw basis、线性一次维数六插入、维数正规化和 $\overline{\mathrm{MS}}$ 口径下，完整一圈反常维数矩阵来自 Jenkins--Manohar--Trott 与 Alonso--Jenkins--Manohar--Trott 的三篇系列计算。本书不重算该矩阵，只使用其结构和部分例子；精确文献分工和 locator 见附录 B。
...
EFT 阶数是逆尺度次数与圈次数的双重分次。固定 bidegree 的扩大算符空间先吸收反项，再投影到物理 quotient；线性 sector 由反常维数矩阵运行，而双插入阶一般需要非线性 RGE。
